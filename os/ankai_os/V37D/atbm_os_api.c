...
{
	int ret =0;	
	tBusHandle.class_code = USB_DEVICE_CLASS_RESERVED;
	tBusHandle.enumok_callback = atbm_usb_probe;
	tBusHandle.discon_callback = atbm_usb_disconnect;
	ret = atbm_usb_register(&tBusHandle);
	if(ret){
		wifi_printk(WIFI_ALWAYS,"USB Register Fail\n");
		return -1;
	}
	/* open the usb bus, this will enumunate all usb descriptor */
	usb_bus_open(USB_MODE_HIGH_SPEED);
	return 0;
}
int atbm_usb_register_deinit()
{
	atbm_usb_deregister(tBusHandle);
}
int atbm_akwifi_setup_usb(unsigned long mode)
{
	/* settint and register usb bus class */	
	usb_bus_open(mode);
...
int atbmusb_net_dhcp_start (struct netif *netif, unsigned int timeout_sec)
{
	...
	while(1) {
		...
		if (get_tick_count() - tick > timeout_sec * 1000) {
			dhcp_stop(netif);
...
int atbm_akwifistation_netif_init(void)
{
	...
	//}
	#endif
//	atbmusb_net_dhcp_start(p_netif,60);
	return 0;
}
/**
