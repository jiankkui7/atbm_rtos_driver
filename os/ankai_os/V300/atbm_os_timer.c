...
static void atbm_timer_hash_del_bytimer(struct OS_TIMER_S *timer)
{
	...
	if(hash_pTimer == ATBM_NULL){
		wifi_printk(WIFI_ALWAYS,"atbm_timer_hash_del_bytimer[%s][%d]hash_pTimer == ATBM_NULL\n",timer->pTimerName,
			timer->TimerHander);
		return;
	}
...
{	
	...
	HASH_UNLOCK();
	if(pTimer == ATBM_NULL){
		wifi_printk(WIFI_ALWAYS,"atbm_TimerOutFunc %d ERROR\n",timer_id);
		if((ATBM_GET_TIMER_IN_START())&&(ATBM_GET_CUR_ID() == -1)){
			ATBM_SET_CUR_ID(timer_id);
		}
		else {
			wifi_printk(WIFI_ALWAYS,"atbm_TimerOutFunc %d ERROR cur\n",timer_id);
		}
		return;
	}
