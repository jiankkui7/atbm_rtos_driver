...
volatile int lwip_queue_enable = 0;
int lwip_enable = 0;
...
err_t atbm_wifi_tx_pkt_netvif(struct netif *netif, struct pbuf *p)
{
	...
	while(!lwip_queue_enable){
		...
		if(retry){
			atbm_SleepMs(5);
//			wifi_printk(WIFI_DBG_ERROR, "packet droped retry:%d\n", retry);
			continue;
		}else{
//			wifi_printk(WIFI_DBG_ERROR, "packet droped retry:%d\n", retry);
			return 0;
...
static atbm_void __atbm_wifi_rx_pkt(struct atbm_net_device *dev, struct atbm_buff *atbm_skb) 
{
	...
	if(netif->input){
		if(netif->input(p,netif)!=ERR_OK){
			atbm_changeThreadPriority(HMAC_MAX_THREAD_PRIO);
//			AK_Relinquish();
			if(retry--){
				atbm_SleepMs(5);
			}else{
