...
extern rt_bool_t rt_wlan_is_ready(void);
...
static void UartCmd_Set_Freq(int argc, char **args){
	char cmd[20] = {0};
	if(argc < 4){
		rt_kprintf("need more parameter,please try again!\n");
		return;
	}
	sprintf(cmd, "set_freq %s %s ", args[2], args[3]);
	rt_kprintf("cmd: %s\n", cmd);
	wsm_write_mib(&g_hw_prv, WSM_MIB_ID_FW_CMD, cmd, strlen(cmd), g_vmac->if_id);
}
...
extern atbm_int32 atbm_wifi_mfg_PT_Test(/*atbm_int32 targetFreq, atbm_int32 rssiFilter, atbm_int32 evmFilter, atbm_int32 cableLoss, */atbm_int32 isWriteEfuse);
...
/* WIFI test cmd start*/
    ...
    {"enable_log",       atbm_enable_logprint},
#endif
    {"NULL", NULL}
};
