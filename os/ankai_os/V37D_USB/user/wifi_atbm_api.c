...
int wifi_scan(wifi_ap_list_t *ap_list)
{
	...
	for(i =0;i < result->count; i++)
	{
		...
		ap_list->ap_info[i].security = info->security;  //?А??бе??
 ...
 */
int wifi_quick_connect(unsigned char *essid,unsigned char *key, struct bss_descriptor *bss_saved)
{
...
{
	atbm_wifi_etf_start_tx(channel, rate_value, is_40M, greedfiled);
}
int wifi_etf_stop_tx()
{
	atbm_wifi_etf_stop_tx();
}
int wifi_etf_start_rx(int channel ,int is_40M)
{
	atbm_wifi_etf_start_rx(channel, is_40M);
}
int wifi_etf_stop_rx()
{
	atbm_wifi_etf_stop_rx();
}
int wifi_mfg_CarrierTone(WLAN_CHANNEL channel)
{
	atbm_wifi_mfg_CarrierTone(channel);
}
