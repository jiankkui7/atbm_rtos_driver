...
#define SDIO_PARTI     "/etc/config/wifi_backup.txt"
#define WIFI_IF_PARTI  "/etc/config/wifi_backup.txt"

#define AK_SUCCESS          0
#define AK_FAILED           -1


extern bool sdio_backup_set_data(char *data_addr,int data_len);
extern bool sdio_backup_get_data(char **data_addr,int *data_len);
...
int wifi_backup_usr_data(int mode, struct wifi_info *info, int len)
{
	...
	int bkup_fd = open(WIFI_IF_PARTI, O_RDWR | O_CREAT, 0666);
	if(bkup_fd < 0)
	{
	...
	{        
		if (mode == MODE_WRITE)
		{
			//再次打开来清空文件，防止文件之前存的长数据污染本次写入的短数据
			close(bkup_fd);
			bkup_fd = open(WIFI_IF_PARTI, O_WRONLY | O_TRUNC);
			ret = write(bkup_fd, info, sizeof(struct wifi_info));
			if(ret != sizeof(struct wifi_info))
			{
			...
			{
				rt_kprintf("write %s ok\n", WIFI_IF_PARTI);
                ret = AK_SUCCESS;
			}
		}
		else if (mode == MODE_READ)
		{
			ret = read(bkup_fd, info, sizeof(struct wifi_info));
            if(ret != sizeof(struct wifi_info))
            {
...
int sdio_backup_flash_value(char mode, char *data, int len)
{
	...
	int bkup_fd = open(SDIO_PARTI, O_RDWR | O_CREAT, 0666);
	if(bkup_fd < 0)
	{
		rt_kprintf("open %s error\n", SDIO_PARTI);
        ret = AK_FAILED;
	...
	else
	{
		if (mode == MODE_WRITE)
		{
			//再次打开来清空文件，防止文件之前存的长数据污染本次写入的短数据
			close(bkup_fd);
			bkup_fd = open(SDIO_PARTI, O_WRONLY | O_TRUNC);
			ret = write(bkup_fd, data, len);
			if(ret < len)
			{
				rt_kprintf("write %s error\n", SDIO_PARTI);
                ret = AK_FAILED;
...
int wifi_backup_sdio_data()
{
	...
	{
		rt_kprintf("get sdio data error\n");
		return -1;
	}
	rt_kprintf("sdio back data len %d\n", sdio_data_len);
	buffer = malloc(sdio_data_len + 4); // sdio data + sdio data size
	if(buffer == NULL)
	{
		rt_kprintf("alloc mem for sdio data error\n");
		return -1;
	}
	...
	{
		rt_kprintf("sdio_backup_flash_value read Error!\n");
	}
	else
	{
		...
		if(buffer_r == NULL)
		{
			rt_kprintf("alloc mem for sdio data resume error\n");
		}
		else
		{
			if(sdio_backup_flash_value(0, buffer_r, sdio_data_len_r + 4) < 0) //read data and data size
			{
				rt_kprintf("Error sdio_backup_flash_value read!\n");
			}
		}
	}
	...
	if(sdio_data_len != sdio_data_len_r || memcmp(buffer, buffer_r, sdio_data_len) != 0)
	{
		...
		{
			rt_kprintf("Error sdio backup data\n");
 		}
		else
		{
