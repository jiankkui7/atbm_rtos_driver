...
atbm_int32 smartconfig_have_may_magic_channel()
{
	return  smt_magic_channel_mask;
}

atbm_void smartconfig_clear_magic_channel()
{
...
FLASH_FUNC atbm_void smartconfig_scan_start()
{
	g_hw_prv.scan_ret.len = 0;	
	g_vmac->scan_expire = 2;
	g_vmac->scan_no_connect_back = 1;
	g_vmac->scan_no_connect = 1;
	/*start smartconfig*/
	g_hw_prv.scan.scan_smartconfig = 1;
	g_hw_prv.st_status = CONFIG_ST_START;

	atbmwifi_scan_start(g_vmac);
	g_hw_prv.st_status = CONFIG_ST_GET_MAGIC;
}

FLASH_FUNC atbm_int32 smartconfig_start(struct smartconfig_config * st_cfg)
{	
	atbm_int32 Istate=0;
	if(st_cfg == ATBM_NULL){
	...
	if((st_cfg->payload_time<500)||(st_cfg->payload_time>30000)){
	 ...
	 atbm_memcpy(&g_hw_prv.st_cfg,st_cfg,sizeof(struct smartconfig_config ));
	 /*disconnect AP*/
...
extern int msh_exec(char *cmd, rt_size_t length);
	msh_exec(cmd, strlen(cmd));
...
#endif
	 atbm_StartTimer(&g_hw_prv.smartconfig_expire_timer,13 * (st_cfg->payload_time + st_cfg->magic_time*14));
	 ...
	 smartconfig_scan_start();
	 return CONFIG_ST_START;
}

FLASH_FUNC atbm_int32 smartconfig_stop(atbm_void)
{
	atbmwifi_stop_smartconfig(&g_hw_prv);
	return g_hw_prv.st_status;
}
FLASH_FUNC atbm_int32 smartconfig_status(atbm_void)
{
	return g_hw_prv.st_status;
}
...
FLASH_FUNC atbm_void atbmwifi_stop_smartconfig(struct atbmwifi_common *hw_priv )
{
	...
	atbm_CancelTimer(&hw_priv->smartconfig_expire_timer);
...
FLASH_FUNC atbm_void smartconfig_start_timer_func(TIMER_1ST_PARAM atbm_void* arg)
{		
	wifi_printk(WIFI_ATCMD,"smartconfig timeout\n");
...
FLASH_FUNC atbm_int32 smartconfig_magic_scan_done(struct atbmwifi_common *hw_priv)
{
	//iot_printf("smartconfig_magic_scan_done %d\n",hw_priv->st_status);
	...
	if(hw_priv->st_status == CONFIG_ST_SWITCH_PAYLOAD_CH ){
		struct wsm_set_chantype arg ;
		{
			...
			arg.flag = 0;			//no use
			arg.channelNumber = hw_priv->st_configchannel;	// channel number
...
//		arg.flag |= BIT(5);
		...
		wifi_printk(WIFI_ALWAYS,"st_status %d\n",hw_priv->st_status);
...
{
	ht40_smt_legacy_data.p_legacy_data = (struct ht40_smt_data *)malloc(sizeof(struct ht40_smt_data )*128);
	ht40_smt_legacy_data.legacy_data_round = (atbm_uint8 *)malloc(sizeof(atbm_uint8)*128);
...
atbm_void smt_put_data_for_sort(atbm_uint8 index, atbm_uint8 len)
{
	...
	printf("data full, try replace index:%d len:%x\n", index, len);
	min_counter_index = 0;
...
atbm_void fromap_sort_and_set_data(atbm_void)
{
	...
	for(u8idx = 0; u8idx< 8; u8idx++)
	{
		...
		{
			printf("Warn ht40_gdata u8idx:%d cnt 0,no data continue\n",u8idx);
			continue;
		}	       
...
atbm_void fromap_merge_smt_data(atbm_uint8 cur_g_indx, atbm_uint8 score_level)
{
	...
	for(i = start_idx; i < end_idx; i++)
	{
		...
		{
			memcpy(&ht40_smt_legacy_data.p_legacy_data[i], &ht40_group_data[i%8], 8);
			ht40_smt_legacy_data.legacy_data_round[i]++;
			printf("merge copy for fist time i:%d\n", i);
	...
	if(update_flag)
	{
		...
		memcpy((atbm_uint8 *)&ht40_group_data[0],  (atbm_uint8 *)&ht40_smt_legacy_data.p_legacy_data[start_idx],  sizeof(struct ht40_smt_data)*8);
		//	memcpy(&ht40_group_data[0], &ht40_smt_legacy_data.p_legacy_data[start_idx], 8*8);
		...
		fromap_sort_and_set_data();
		//update legacy data
		memcpy(&ht40_smt_legacy_data.p_legacy_data[start_idx], &ht40_group_data[0], sizeof(struct ht40_smt_data)*8);
		...
		if((crc8&0x7f) == group_payload[7])  //only check low 7 bit of CRC8 value
		{
			...
			*/		
			...
			memcpy(&smt_payload_data[data_pos], group_payload, 8);				
...
static atbm_int32 atbm_smt_parse_payload(struct atbmwifi_common *hw_priv);
static atbm_int32 smt_fun_recv_payload(struct atbmwifi_common *hw_priv, atbm_uint8 smt_type, atbm_uint16 rxdata_raw)
{
...
//			wifi_printk(WIFI_ALWAYS, "!!!!!!!!!group %d done group_mask:%x!!!!!!!!!!!!\n", cur_group_index, group_mask);
			...
			{
				atbm_smt_parse_payload(hw_priv);
			}
		}else
		{
...
{	
	...
	bitarrays = (atbm_uint8*)malloc(512);
	bit2smt_data = (atbm_uint8*)malloc(128);
	smt_bits_data = (atbm_uint8*)malloc(128);
	...
	*/	
	...
	{		
		...
		{
			printf("Error length, Smartconfig done XOR check passed!\n");
			free(bit2smt_data);
			bit2smt_data = NULL;
			return 0;
		}

		memcpy(smt_ssid, &bit2smt_data[0],ssid_len);
		memcpy(smt_pwd,  &bit2smt_data[ssid_len],pwd_len);
		//Command_SetPolling(1);		
		//printf(" \n");
		wifi_printk(WIFI_ALWAYS, "XOR passed! ssid :%s pwd:%s\n",smt_ssid,  smt_pwd);
		//printf(" \n");
		memset(hmac_cfg_p->password,0,sizeof(hmac_cfg_p->password));		
		memset(hmac_cfg_p->ssid,0,sizeof(hmac_cfg_p->ssid));


		hmac_cfg_p->password_len = pwd_len;		
		memcpy(hmac_cfg_p->password,smt_pwd, pwd_len);    		

		hmac_cfg_p->ssid_len = ssid_len;
		memcpy(hmac_cfg_p->ssid,smt_ssid, ssid_len);


		iot_printf("password_len %d ssidlen %d pwd:%s \n",hmac_cfg_p->password_len,hmac_cfg_p->ssid_len,hmac_cfg_p->password);
		if(hmac_cfg_p->password_len >= 8){
			hmac_cfg_p->key_mgmt = ATBM_KEY_WPA2;
			hmac_cfg_p->privacy =1;
		}
		else if(hmac_cfg_p->password_len==5){
			hmac_cfg_p->key_mgmt = ATBM_KEY_WEP;
			hmac_cfg_p->privacy =1;
		}
		else if(hmac_cfg_p->password_len==13){
			hmac_cfg_p->key_mgmt = ATBM_KEY_WEP;
			hmac_cfg_p->privacy =1;
		}
		else if(hmac_cfg_p->password_len==0) {
			wifi_printk(WIFI_ALWAYS, "EY_NONE!!");   
			hmac_cfg_p->key_mgmt = ATBM_KEY_NONE;
			hmac_cfg_p->privacy =0;
		}
		else {
			wifi_printk(WIFI_ALWAYS, "<ERROR> smartconfig key len error %d\n",hmac_cfg_p->password_len);
		}
...
//		atbmwifi_flash_param_wificonfig_change();
				//connect AP		
		atbmwifi_wpa_event_queue((atbm_void*)g_vmac,(atbm_void*)(hmac_cfg_p),
			ATBM_NULL,WPA_EVENT__SMARTCONFIG_SUCCESS,ATBM_WPA_EVENT_NOACK);
				//event
//		atbmwifi_event_indicate(0,WIFI_SMARTCONFIG_SUCCESS);		
		return 1;
	}
...
atbm_void fromap_smartconfig_start() 
{	 
...
atbm_int32 atbm_smt_ht_rx_handler(struct atbmwifi_common *hw_priv,char * data,int rx_mac_len,int channel, int rssi)
{
...
//ew_rx_last_time= hal_get_os_second_time();
	...
	if(ht_state  == (int)(HT_CONFIG_ST_GET_MAGIC)){	
		...
		if(smt_magic_rev_type){
			if(g_hw_prv.st_status < CONFIG_ST_SWITCH_PAYLOAD_CH){
				g_hw_prv.st_status = CONFIG_ST_SWITCH_PAYLOAD_CH;//compatible with atbm smartconfig
			...
			//ew_rx_last_time= hal_get_os_second_time();
			if(g_hw_prv.st_status == CONFIG_ST_GET_PAYLOAD)
			{
		...
		smt_fun_recv_payload(hw_priv, smt_type, rx_mac_len);
	}	
	return 0;
}
...
atbm_int32 smartconfig_start_rx(struct atbmwifi_common *hw_priv,struct atbm_buff *skb,int channel )
{
//	wifi_printk(WIFI_ALWAYS, "smartconfig_rx1 len %d,st %d\n",ATBM_OS_SKB_LEN(skb),hw_priv->st_status);
	...
	rssi = hw_hdr->signal;
    //printf("rssi:%d\n", rssi);
	atbm_smt_ht_rx_handler(hw_priv,(char *) ATBM_OS_SKB_DATA(skb), ATBM_OS_SKB_LEN(skb),channel, rssi);
...
FLASH_FUNC atbm_void atbm_wifi_smart_config_rx(struct atbmwifi_common *hw_priv,struct atbm_buff *skb,int channel )
{		
...
FLASH_FUNC atbm_void atbmwifi_monitor_enable()
{
...
FLASH_FUNC atbm_void atbmwifi_monitor_disable()
{
	struct wsm_reset reset;
	
	reset.reset_statistics = ATBM_TRUE;
	/*start smartconfig*/
	g_hw_prv.scan.scan_smartconfig = 0;
	g_hw_prv.st_status = CONFIG_ST_IDLE;
	wsm_reset(&g_hw_prv,&reset,0);
}

#define WSM_SET_CHANTYPE_STARTCONFIG		BIT(4)
FLASH_FUNC atbm_int32 atbmwifi_monitor_channel(atbm_int32 channelnum)
{
	struct atbmwifi_common *hw_priv = _atbmwifi_vifpriv_to_hwpriv(g_vmac);

			
	if((channelnum < 1) || (channelnum > 13 )){
		iot_printf("invalid channel number,range [1~13]\n");
		return -1;
	}
	//
	g_vmac->scan_expire = 2;
...
atbm_void smartconfig_success_notify(struct atbmwifi_vif *priv, struct atbmwifi_cfg *hmac_cfg_p){
	struct atbmwifi_common *hw_priv = _atbmwifi_vifpriv_to_hwpriv(priv);

	atbmwifi_stop_smartconfig(hw_priv);
	hw_priv->st_status = CONFIG_ST_DONE_SUCCESS;
	atbmwifi_event_uplayer(priv, ATBM_WIFI_SMARTCONFIG_SUCCESS, hmac_cfg_p);
}
