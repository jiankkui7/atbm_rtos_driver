...
#if WLAN_ZERO_COPY
	skb = atbm_dev_alloc_os_skb(len);
#else
	skb =  __atbm_pbuf_alloc(len);
#endif //WLAN_ZERO_COPY
	if(skb == ATBM_NULL){
		ATBM_BUG_ON(1);
		return  ATBM_NULL;
	}
...
struct atbm_buff * atbm_dev_alloc_skbhdr() 
{
	struct atbm_buff *p;
 ...
 */
/*
static atbm_void __skb_queue_after(struct atbm_buff_head *list,
...
*/
 ...
 static  atbm_void __skb_queue_before(struct atbm_buff_head *list,
...
/*
static  atbm_void __skb_queue_head(struct atbm_buff_head *list,
				    struct atbm_buff *newsk)
{
	__skb_queue_after(list, (struct atbm_buff *)list, newsk);
}
*/
 ...
 *	A buffer cannot be placed on two lists at the same time.
 */
static  atbm_void __skb_queue_tail(struct atbm_buff_head *list,
				   struct atbm_buff *newsk)
{
	__skb_queue_before(list, (struct atbm_buff *)list, newsk);
}
static  atbm_void __skb_unlink(struct atbm_buff *skb, struct atbm_buff_head *list)
{
 ...
 */
/*
static  struct atbm_buff *__skb_dequeue_tail(struct atbm_buff_head *list)
{
 ...
 *	returned or %NULL if the list is empty.
 */
static  struct atbm_buff *__skb_dequeue(struct atbm_buff_head *list)
{
