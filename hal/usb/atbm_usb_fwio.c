...
#if ATBM_HW_CHECKSUM
#include "firmware_ares_usb_notxconfirm_rxmulti_hwchecksum.h"
#else
#include "firmware_ares_usb_notxconfirm_rxmulti.h"
#endif
#else
#if ATBM_HW_CHECKSUM
#include "firmware_ares_usb_notxconfirm_hwchecksum.h"
#else
...
static int atbm_load_firmware_generic(struct atbmwifi_common *priv, atbm_uint8 *data,atbm_uint32 size,atbm_uint32 addr)
{
	...
	//iot_printf("atbm_load_firmware_generic++ addr 0x%x,size %d\n",addr,size);
	...
	for (put = 0; put < size ;put += DOWNLOAD_BLOCK_SIZE) {
		...
		if (ret < 0) {
			...
			iot_printf(ATBM_KERN_ERR "%s:put = 0x%x\n",__FUNCTION__,put);
			goto error;
		}
