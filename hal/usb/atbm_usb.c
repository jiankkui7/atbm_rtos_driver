...
int atbm_usb_sw_read_port(struct sbus_priv *self, atbm_uint32 addr, atbm_void *pdata,int len)
{
...
#if CONFIG_USB_AGGR_URB_TX
atbm_uint8 *atbm_usb_pick_txDMABuf(struct sbus_priv *self)
{
	unsigned long flags=0;
	atbm_uint8 *buf;
...
atbm_uint8 *atbm_usb_get_txDMABuf(struct sbus_priv *self)
{
	atbm_uint8 *buf;
	unsigned long flags=0;
	atbm_spin_lock_irqsave(&self->lock, flags);
...
void atbm_usb_free_txDMABuf_all(struct sbus_priv *self,atbm_uint8 * buffer,int cnt)
{
	unsigned long flags=0;
	atbm_spin_lock_irqsave(&self->lock, flags);
	ATBM_WARN_ON_FUNC(cnt==0);
	if(buffer != self->drvobj->NextFreePost){
...
//	tx_urb->pallocated_buf_len = tx_len < PER_PACKET_LEN ? tx_len : tx_urb->pallocated_buf_len;
	wifi_printk(WIFI_IF, "pallocated_buf:%p txdmabuff:%p usb_aggr_buff:%p tx_urb->pallocated_buf_len:%d\n", tx_urb->pallocated_buf, txdmabuff, usb_aggr_buff, tx_urb->pallocated_buf_len);
//	dump_mem(tx_urb->pallocated_buf, tx_len);
...
atbm_void  atbm_usb_urb_put(struct sbus_priv *self,atbm_uint32 *bitmap,int id)
{
	unsigned long flags=0;
	//if(!(*bitmap & BIT(id))){
	//	iot_printf("atbm_usb_urb_put id %d,map %x\n",id,*bitmap);
	//}
	atbm_spin_lock_irqsave(&self->lock, flags);
	*bitmap &= ~BIT(id);
	atbm_spin_unlock_irqrestore(&self->lock, flags);
...
atbm_void  atbm_usb_receive_data_complete( atbm_urb_s *atbm_urb)
{
	...
	if((self->drvobj->suspend_skb_len != 0)&&(self->drvobj->suspend_skb != ATBM_NULL))
	{
		if(atbm_skb_tailroom(self->drvobj->suspend_skb)<self->drvobj->suspend_skb_len+RecvLength)
		{
			...
			iot_printf(ATBM_KERN_ERR "suspend skb len is not enough(%d),(%ld)\n",
		...
		iot_printf(ATBM_KERN_ERR "suspend rebuild\n");
	}
...
#if ATBM_IMMD_RX
	{
		extern int atbm_rx_bh_cb(struct atbmwifi_common *hw_priv,struct atbm_buff *skb);
	...
	if(atbm_skb_queue_len(&hw_priv->rx_frame_queue) <=1){
		/*RxBuffer Have Packet,need to receive,wakeup bh rx*/
		if (atbm_atomic_add_return(1, &hw_priv->bh_rx) == 1){
			//wifi_printk(WIFI_DBG_ERROR, "%s atbm_os_wakeup_event++\n",__func__);
...
static int atbm_usb_off(const struct atbm_platform_data *pdata)
{
	...
	//	ret = pdata->insert_ctrl(pdata, ATBM_FALSE);
	return ret;
}
...
static int atbm_usb_on(const struct atbm_platform_data *pdata)
{
   ...
   // if (pdata->insert_ctrl)
	//	ret = pdata->insert_ctrl(pdata, ATBM_TRUE);
				   ...
				   const struct atbm_usb_device_id *id)
{
	...
	wifi_printk(WIFI_ALWAYS,"pusbdev =0x%p",dvobj->pusbdev);
...
atbm_void atbm_core_release(struct atbmwifi_common *hw_priv)
{
	...
	/*Flush the queue */
	__atbm_flush(hw_priv, ATBM_TRUE, g_vmac->if_id);
	/*Free TxQueue Timer*/
	for (queue_id = 0; queue_id < 4; ++queue_id){
		atbmwifi_queue_deinit(&hw_priv->tx_queue[queue_id]);
	}
	atbm_FreeTimer(&g_vmac->scan_expire_timer);
	atbm_FreeTimer(&hw_priv->smartconfig_expire_timer);
	atbm_FreeTimer(&g_vmac->connect_expire_timer);
	atbm_FreeTimer(&g_vmac->chswitch_timer);
	/*Free wpa supplicant mem*/
	free_wpa_supplicant(g_vmac);
	/*Destory tx cmd buffer*/	
	wsm_buf_deinit(&hw_priv->wsm_cmd_buf);
	/*Free netdev*/
	atbm_free_netdev(g_vmac->ndev);
	AtbmwifiRdy =0;
	...
	if(hw_priv->vif_list[0]) {
		struct atbmwifi_vif *priv = hw_priv->vif_list[0];
