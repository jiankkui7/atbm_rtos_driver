...
FLASH_FUNC static  atbm_void __atbmwifi_queue_lock(struct atbmwifi_queue *queue,struct atbmwifi_vif *priv)
{
	if (!priv){
		return ;
	}
...
FLASH_FUNC static  atbm_void atbmwifi_queue_parse_id(atbm_uint32 packetID, atbm_uint8 *queue_generation,
						atbm_uint8 *queue_id,
...
FLASH_FUNC static  atbm_uint32 atbmwifi_queue_make_packet_id(atbm_uint8 queue_generation, atbm_uint8 queue_id,
...
FLASH_FUNC int atbmwifi_queue_stats_init(struct atbmwifi_queue_stats *stats,
			    ...
			    struct atbmwifi_common *hw_priv)
{
	...
	for (i = 0; i < ATBM_WIFI_MAX_VIFS; i++) { 
		stats->link_map_cache[i] =atbm_kzalloc(sizeof(int[WLAN_LINK_ID_MAX]),GFP_KERNEL);
...
static atbm_void atbmwifi_queued_timeout(TIMER_1ST_PARAM atbm_void *arg)
{
	if(arg==ATBM_NULL)
		return;
	struct atbmwifi_queue *queue =(struct atbmwifi_queue *)arg;
	unsigned long flags;
	...
	while (!atbm_list_empty(&queue->queue)) {
		...
		if ((atbm_GetOsTimeMs()- item->queue_timestamp) < queue->ttl){
			wifi_printk(WIFI_QUEUE,"queued_to not remove..%d..%d\n",atbm_GetOsTimeMs(), (unsigned int)item->queue_timestamp);
			break;
		}
		txpriv = &item->txpriv;
		skb=item->skb;
		...
		atbm_spin_lock_bh(&stats->lock);
		--stats->num_queued[if_id];
		atbm_spin_unlock_bh(&stats->lock);
	...
	if (!atbm_list_empty(&queue->queue)) {
		atbm_StartTimer(&queue->timeout, 5*HZ);
		queue->queuedFlag=ATBM_TRUE;
	}
	atbm_spin_unlock_irqrestore(&queue->lock,flags);
}
FLASH_FUNC int atbmwifi_queue_init(struct atbmwifi_queue *queue,
		      ...
		      atbm_size_t capacity)
{
	...
	queue->pool = atbm_kzalloc(sizeof(struct atbmwifi_queue_item) * capacity /*pool*/
...
FLASH_FUNC int atbmwifi_queue_clear(struct atbmwifi_queue *queue, int if_id)
{
	int i, cnt, iter;
	struct atbmwifi_queue_stats *stats = queue->stats;
	unsigned long flags;
...
FLASH_FUNC atbm_void atbmwifi_queue_deinit(struct atbmwifi_queue *queue)
{
	int i;
	struct atbmwifi_queue_stats *stats = queue->stats;
	
	atbmwifi_queue_clear(queue, ATBM_WIFI_ALL_IFS);
	atbm_FreeTimer(&queue->timeout);
	ATBM_INIT_LIST_HEAD(&queue->free_pool);
	...
	for (i = 0; i < ATBM_WIFI_MAX_VIFS; i++) {
		if (stats->link_map_cache[i]) {
			atbm_kfree(stats->link_map_cache[i]);
			stats->link_map_cache[i] = ATBM_NULL;
		}
	}
...
FLASH_FUNC atbm_size_t atbmwifi_queue_get_num_queued(struct atbmwifi_vif *priv,
				   ...
				   atbm_uint32 link_id_map)
{
	...
	if (atbm_likely(link_id_map == (atbm_uint32) -1)) {
		...
		wifi_printk(WIFI_PS,"--->muticast num  ret = %d\n",ret);
...
FLASH_FUNC int atbmwifi_queue_put(struct atbmwifi_queue *queue,
...
*/
FLASH_FUNC int atbmwifi_queue_get(struct atbmwifi_queue *queue,
			int if_id,
		     atbm_uint32 link_id_map,
		     struct wsm_tx **tx,
		     struct atbmwifi_txpriv **txpriv)
...
FLASH_FUNC int atbmwifi_queue_requeue(struct atbmwifi_queue *queue, atbm_uint32 packetID)
{
	int ret = 0;
...
FLASH_FUNC int atbmwifi_queue_requeue_all(struct atbmwifi_queue *queue)
{
	struct atbmwifi_queue_stats *stats = queue->stats;
	unsigned long flags;
...
FLASH_FUNC int atbmwifi_queue_remove(struct atbmwifi_queue *queue, atbm_uint32 packetID)
{
	int ret = 0;
	atbm_uint8 queue_generation, queue_id, item_generation, item_id, if_id, link_id;
	struct atbmwifi_queue_item *item;
...
FLASH_FUNC int atbmwifi_queue_get_skb(struct atbmwifi_queue *queue, atbm_uint32 packetID,
			 struct atbm_buff **skb,
...
FLASH_FUNC atbm_void atbmwifi_queue_lock(struct atbmwifi_queue *queue,struct atbmwifi_vif *priv)
{
	unsigned long flags;
	atbm_spin_lock_irqsave(&queue->lock,flags);
...
FLASH_FUNC atbm_void atbmwifi_queue_unlock(struct atbmwifi_queue *queue,struct atbmwifi_vif *priv)
{
	unsigned long flags;
	atbm_spin_lock_irqsave(&queue->lock,flags);
	__atbmwifi_queue_unlock(queue,priv);
...
FLASH_FUNC ATBM_BOOL atbmwifi_queue_stats_is_empty(struct atbmwifi_queue_stats *stats,
