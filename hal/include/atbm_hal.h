...
#define WLAN_LINK_ID_MAX				(ATBMWIFI__MAX_STA_IN_AP_MODE + 3)
...
/////////////from MSTAR head file wlan_ATBM.h
#include "atbm_type.h"
#include "atbm_config.h"
...
struct atbmwifi_common{	
    ...
    /*country XXX*/
	atbm_uint8 			   country_id;
	atbm_uint8 			   country_code[3];
	/*config*/
	atbm_uint8 				channel_idex;
	atbm_uint8 				channel_type;	//support
	...
	int       		if_id_selected;
	atbm_uint32				key_map;
	struct atbmwifi_key_t		keys[WSM_KEY_MAX_INDEX + 1];
...
int atbmwifi_autoconnect(struct atbmwifi_vif *priv);
...
int smartconfig_start_rx(struct atbmwifi_common *hw_priv,struct atbm_buff *skb,int channel);
...
unsigned int cfg80211_classify8021d(struct atbm_buff *skb);
...
FLASH_FUNC int atbmwifi_sta_del(struct atbmwifi_vif *priv, atbm_uint8 * staMacAddr);
...
/***key**/
FLASH_FUNC atbm_void atbmwifi_del_key(struct atbmwifi_vif *priv,int pairwise,int linkid);
FLASH_FUNC int atbmwifi_set_key(struct atbmwifi_vif *priv,int pairwise,int linkid);
/***main**/
...
//
FLASH_FUNC int atbmwifi_get_hard_linked_macs(struct atbmwifi_vif *priv,  atbm_uint8 *mac, atbm_uint32 maclen);
