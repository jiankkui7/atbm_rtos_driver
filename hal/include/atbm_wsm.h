...
static __INLINE int wsm_set_efuse_mac_address(struct atbmwifi_common *hw_priv, atbm_uint8 *mac)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_EFUSE_MAC, mac,
			    ATBM_ETH_ALEN,-1);
}

static __INLINE int wsm_set_efuse_data(struct atbmwifi_common *hw_priv, atbm_uint8 *efuse, int len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_EFUSE_DATA, efuse,
			    len,-1);
}
struct efuse_headr{
	atbm_uint8 specific;
...
static inline int wsm_get_SIGMSTAR_256BITSEFUSE(struct atbmwifi_common *hw_priv, atbm_uint8 *efuse, atbm_int32 len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_SIGMSTAR_256BITSEFUSE, efuse,
			    len,-1);
}
static inline int wsm_set_SIGMSTAR_256BITSEFUSE(struct atbmwifi_common *hw_priv, atbm_uint8 *efuse, atbm_int32 len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_SIGMSTAR_256BITSEFUSE, efuse,
			    len,-1);
}

static __INLINE int wsm_get_efuse_data(struct atbmwifi_common *hw_priv, atbm_void *efuse, int len)
{
