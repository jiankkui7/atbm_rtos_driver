...
FLASH_FUNC int atbmwifi_rx_filter_retry(struct atbmwifi_vif *priv,atbm_uint8 link_id,struct atbm_buff *skb);
FLASH_FUNC int atbmwifi_ieee80211_rx(struct atbmwifi_vif *priv,struct atbm_buff *skb);
FLASH_FUNC int __atbmwifi_ieee80211_rx(struct atbmwifi_vif *priv,struct atbm_buff *skb);
extern atbm_uint8 atbm_wmm_status_get(atbm_void);

static atbm_int32 Rssi_last = 0;
static atbm_int32 Rssi_aver = 0;

FLASH_FUNC atbm_int32 atbmwifi_get_rssi_avg(atbm_void)
{
	return -(Rssi_aver >> 10);
}

FLASH_FUNC atbm_int32 atbmwifi_get_rssi(atbm_void)
{
	return Rssi_last;
}

FLASH_FUNC static atbm_void atbmwifi_set_rssi(atbm_int8 rssi)
{
	Rssi_last = rssi;
...
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_tx_time(atbm_uint32 time_period, atbm_uint32 time_transmit)
{
	atbm_int32 ret = 0;
	char cmd[32];
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_tx_time(), error\n");
		return -ATBM_EINVAL;
	}
	...
	wifi_printk(WIFI_ALWAYS, "atbm: set_tx_time cmd--%s\n", cmd);
...
Return: Success(0), Error(<0)
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_retry(atbm_uint32 retry_num, atbm_uint32 retry_time_ms)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_retry(), error\n");
		return -ATBM_EINVAL;
	}
	...
	wifi_printk(WIFI_ALWAYS, "atbm: set_retry cmd--%s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
Return: Success(0), Error(<0)
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_txpower_mode(atbm_uint32 txpower_idx)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_txpower_mode(), error\n");
		return -ATBM_EINVAL;
	}
	...
	if(txpower_idx > 12){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_txpower_mode(), error, txpower_idx %d\n", txpower_idx);
	}

	atbm_memset(cmd, 0, sizeof(cmd));
	sprintf(cmd,"set_txpower_mode %d ", txpower_idx);
	wifi_printk(WIFI_ALWAYS, "atbm: set_txpower_mode cmd--%s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
Return: Success(0), Error(<0)
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_txpower(atbm_uint32 txpower_flag)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_txpower(), error\n");
		return -ATBM_EINVAL;
	}
	
	hw_priv=g_vmac->hw_priv;

	if(txpower_flag != 0 && txpower_flag != 15 && txpower_flag != 63){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_txpower(), error, txpower_flag %d\n", txpower_flag);
		return -1;
	}

	atbm_memset(cmd, 0, sizeof(cmd));
	sprintf(cmd,"set_txpower %d ", txpower_flag);
	wifi_printk(WIFI_ALWAYS, "atbm: set_txpower cmd--%s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
FLASH_FUNC atbm_int32 atbmwifi_set_tx_rate(atbm_int32 rate)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbm: wifi driver failed, please try again!\n");
		return -ATBM_EINVAL;
	}
	...
	wifi_printk(WIFI_ALWAYS, "atbm: rate cmd--%s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
Return: Success(0), Error(<0)
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_sgi(atbm_uint32 sgi)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_sgi(), error\n");
		return -ATBM_EINVAL;
	}
	...
	wifi_printk(WIFI_ALWAYS, "atbm: lmac_sgi cmd--%s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
Return: Success(0), Error(<0)
******************************************************/
FLASH_FUNC atbm_int32 atbmwifi_set_rate_txpower_mode(atbm_int32 txpower_idx)
{
	atbm_int32 ret = 0;
	char cmd[32];
	struct atbmwifi_vif *vif=g_vmac;
	...
	if(g_vmac == ATBM_NULL){
		wifi_printk(WIFI_DBG_ERROR, "atbmwifi_set_rate_txpower_mode(), error\n");
		return -ATBM_EINVAL;
	}
	...
	wifi_printk(WIFI_ALWAYS, "atbm cmd: %s\n", cmd);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, strlen(cmd), vif->if_id);
...
atbm_void atbm_netrx_task(struct atbmwifi_vif *priv)
{
	...
	do{
		...
		if(skb->Type == RX_SKB__TX_STATUS){
			atbmwifi_ieee80211_tx_status(priv,skb, ATBM_NULL);
			needfree = 1;
		}
...
FLASH_FUNC int atbm_rx_task_work(atbm_void *work)
{
...
FLASH_FUNC int atbmwifi_ieee80211_rx_irqsafe(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
...
#ifdef ATBM_RX_STATUS_USE_QUEUE
FLASH_FUNC int atbmwifi_ieee80211_tx_status_irqsafe(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
...
static atbm_void atbm_reorder_pkg_timeout(TIMER_1ST_PARAM atbm_void*arg)
{
	...
	while(i<tid_params->wind_size) 
	{
		...
		if(!atbm_TimeAfter(tid_params->frame_rx_time[index]+(5*HZ/1000))){
	...
	index = BUFF_INDEX_IS_SAFE(tid_params->start_seq);
 	if(tid_params->skb_buffed){		
		...
		atbm_StartTimer(&tid_params->overtime_timer, tid_params->timeout);
 	}
	else {
...
int atbm_reorder_skb_queue(struct atbmwifi_vif *priv,struct atbm_buff *skb,atbm_uint8 link_id)
{
	...
	*/
	...
	{
		//iot_printf("tid_params == NULL");
		return 0;
	}
	if(!atbm_test_bit(BAR_TID_EN,&tid_params->tid_en))
	{
		//iot_printf("link_id(%d),tid(%d),is not enable\n",link_id,tid);
		return 0;
	}
	if(atbmwifi_ieee80211_is_qos_nullfunc(frame->frame_control))
	{
		//iot_printf("qos_nullfunc\n");
		return 0;
	}
...
//		reorder_debug(REORDER_DEBUG,"header(%d),ampdu reordered,buffed(%d)---2\n",tid_params->index_hread,tid_params->skb_buffed);
	...
	{
		atbm_clear_bit(REORDER_TIMER_RUNING,&tid_params->timer_running);
		atbm_CancelTimer(&tid_params->overtime_timer);
	}
	res = -1;
exit_reorder:
	tid_params_spin_unlock(&tid_params->skb_reorder_spinlock,atbm_os_mutexUnLock);
...
static atbm_void atbmwifi_ieee80211_rx_h_sta_process(struct atbmwifi_vif *priv,struct atbmwifi_ieee80211_hdr *hdr) 
{
	...
	//wifi_printk(WIFI_CONNECT,"atbmwifi_ieee80211_rx_h_sta_process-->link_id=%d,sta_asleep_mask=%0x fc %x\n",sta_priv->link_id,priv->sta_asleep_mask,hdr->frame_control);
	if(sta_priv) {		
		/*
...
static int atbmwifi_rx_uapsd_and_pspoll(struct atbmwifi_vif *priv,struct atbmwifi_ieee80211_hdr * hdr)
{
	...
	if(((priv->sta_asleep_mask &BIT(link_id))==0)){
		...
		if (atbmwifi_ieee80211_is_pspoll(fc)){
			...
			if(atbm_test_bit(WLAN_STA_PS,&priv->link_id_db[link_id-1].sta_priv.flags)){
				return RX_CONTINUE;
			}
			...
			/*It's tiger Frame,FrameCtrl|=PM bit 0|qosData|NullData*/
						...
						atbmwifi_ieee80211_is_data_qos(fc))){ /*do u-apsd process*/
			/* if we are in a service period, do nothing */
			if(atbm_test_bit(WLAN_STA_SP,&priv->link_id_db[link_id-1].sta_priv.flags)){
				return RX_CONTINUE;
			}
...
FLASH_FUNC int atbmwifi_ieee80211_tx_status(struct atbmwifi_vif *priv,struct atbm_buff *skb, struct wsm_tx_confirm *arg)
{
	...
	wifi_printk(WIFI_DBG_MSG,"atbm: atbmwifi_ieee80211_tx_status()\n");
	if((priv->iftype == ATBM_NL80211_IFTYPE_AP)||(priv->iftype == ATBM_NL80211_IFTYPE_P2P_GO) || (priv->iftype == ATBM_NL80211_IFTYPE_P2P_CLIENT))
	{
...
extern atbm_void hostapd_eap_wsc_init(struct atbmwifi_vif *priv,atbm_uint8 *da);
...
#ifdef CONFIG_P2P
			else if(stype == ATBM_IEEE80211_STYPE_ACTION){
				struct atbmwifi_ieee80211_mgmt *mgmt = (struct atbmwifi_ieee80211_mgmt *)ATBM_OS_SKB_DATA(skb);
	...
	{		
		//wifi_printk(WIFI_TX, "[TX] b_eapol %d.\n",tx->b_eapol);
		if(tx->b_eapol){
			wpa_supplicant_eapol_notice_ack(priv);
		}
	}
...
FLASH_FUNC atbm_void atbmwifi_tx_confirm_cb(struct atbmwifi_common *hw_priv,
...
/*
FLASH_FUNC static atbm_void atbmwifi_tx_h_calc_tid(struct atbmwifi_vif *priv,
		     struct atbmwifi_txinfo *t)
{
	if (atbmwifi_ieee80211_is_data_qos(t->hdr->frame_control)) {
		atbm_uint8 *qos = atbmwifi_ieee80211_get_qos_ctl(t->hdr);
		t->txpriv.tid = qos[0] & ATBM_IEEE80211_QOS_CTL_TID_MASK;
	} else if (atbmwifi_ieee80211_is_data(t->hdr->frame_control)) {
		t->txpriv.tid = 0;
	}
}
*/

static atbm_void  atbmwifi_notify_buffered_tx(struct atbmwifi_vif *priv, int link_id, atbm_uint32 tid)
{
...
atbmwifi_tx_h_crypt(struct atbmwifi_vif *priv,struct atbmwifi_txinfo *t)
{
	...
	//int encrype = 0;
	int entryIndex;
	entryIndex = atbm_get_key(priv,!atbm_is_multicast_ether_addr(t->hdr->addr1),t->txpriv.link_id);
	if((entryIndex != ATBM_INVALID_KEY) &&
		...
		wifi_printk(WIFI_TX,"%s: encrype \n",__FUNCTION__);
	}
	else {
		return 0;
	}
	switch(atbm_get_crypto(priv,!atbm_is_multicast_ether_addr(t->hdr->addr1))){
...
/* Add WSM header */
FLASH_FUNC static struct wsm_tx *
...
FLASH_FUNC static int
atbmwifi_tx_h_rate_build(struct atbmwifi_common *hw_priv,
			struct atbmwifi_vif *priv,
			struct atbmwifi_txinfo *t,
			struct wsm_tx *wsm)
{
//	ATBM_BOOL tx_policy_renew = ATBM_FALSE;
		...
		case ATBM_NL80211_IFTYPE_STATION:
			if (!(t->tx_info->flags & ATBM_IEEE80211_TX_CTL_USE_MINRATE)){
				if(t->tx_info->flags & ATBM_IEEE80211_TX_CTL_NO_CCK_RATE){
					tx_rate = RATE_INDEX_A_6M;
				}else if(priv->bss.rate.ht_cap.ht_supported == ATBM_TRUE){
...
{	
	...
	//wifi_printk(WIFI_CONNECT,"0---0 Tx dhcp packet %x %x\n",atbm_htons(ETH_P_IP),ethertype);
...
int atbmwifi_rx_dhcp_frame(atbm_uint8 *payload)
{
	...
	//struct ieee80211_hdr *frame = (struct ieee80211_hdr *)skb->data;
	//wifi_printk(WIFI_CONNECT,"0---0 Rx dhcp packet %x %x\n",htons(ETH_P_IP),ethertype);
...
atbm_void atbmwifi_tx(struct atbmwifi_common *hw_priv, struct atbm_buff *skb,struct atbmwifi_vif *priv)
{
	...
	wsm->flags |= flags;
	ret = atbmwifi_tx_h_rate_build(hw_priv,priv, &t, wsm);
	if (ret){		
		ret = -11;
		goto drop;
	}	
...
FLASH_FUNC atbm_void atbmwifi_skb_dtor(struct atbmwifi_common *hw_priv,
		     struct atbm_buff *skb,
		     const struct atbmwifi_txpriv *txpriv)
{
	struct atbmwifi_vif *priv =_atbmwifi_hwpriv_to_vifpriv(hw_priv, txpriv->if_id);
	if (priv&& (priv->iftype==ATBM_NL80211_IFTYPE_AP) && txpriv->raw_link_id) {
		atbmwifi_notify_buffered_tx(priv,txpriv->raw_link_id,txpriv->tid);
	}
	#ifndef ATBM_RX_STATUS_USE_QUEUE
	atbm_dev_kfree_skb(skb);
	#else
...
FLASH_FUNC int atbmwifi_rx_filter_retry(struct atbmwifi_vif *priv,atbm_uint8 link_id,struct atbm_buff *skb)
{
	...
	if (sta_link_id_db->sta_retry.last_rx_seq[status->seqno_idx] ==
		...
		if( atbmwifi_ieee80211_has_retry(hdr->frame_control) ){
			...
			wifi_printk(WIFI_DBG_MSG, "[rx_retry1](%x)(%d)(%d(%d,(%x,%x) skb %p len %d %p\n",hdr->frame_control,
										priv->if_id,link_id,status->seqno_idx,
										sta_link_id_db->sta_retry.last_rx_seq[status->seqno_idx],hdr->seq_ctrl,skb,skb->dlen,ATBM_OS_SKB_DATA(skb));
		}
	}
...
int atbmwifi_class3_err(struct atbmwifi_vif *priv, struct atbmwifi_ieee80211_hdr *frame, struct wsm_rx *arg, int link_id)
{
...
FLASH_FUNC atbm_void atbmwifi_rx_cb(struct atbmwifi_vif *priv,
		  struct wsm_rx *arg,
		  struct atbm_buff **skb_p,int link_id)
{
	...
	//struct atbmwifi_ieee80211_hdr *frame = (struct atbmwifi_ieee80211_hdr *)OS_SKB_DATA(skb) ;
	struct atbmwifi_ieee80211_mgmt *mgmt = (struct atbmwifi_ieee80211_mgmt *)ATBM_OS_SKB_DATA(skb);
...
FLASH_FUNC atbm_void atbmwifi_tx_start(struct atbm_buff *skb,struct atbmwifi_vif *priv)
{
	...
	if(atbmwifi_is_sta_mode(priv->iftype)){
		...
		if(!atbm_is_multicast_ether_addr(ATBM_OS_SKB_DATA(skb))){
			if(!atbmwifi_sta_find(priv,ATBM_OS_SKB_DATA(skb))){
				atbm_dev_kfree_skb(skb);			
				//wifi_printk(WIFI_DBG_ERROR,"atbmwifi_tx_start sta not found \n");
				return;
			}
		}
	}

	wmm_sta = atbm_wmm_status_get();
	res = atbmwifi_ieee80211_data_from_8023(skb,priv->mac_addr,
			    priv->iftype,priv->bssid, wmm_sta,priv->connect.encrype);
	if(res<0)
	{
		wifi_printk(WIFI_CONNECT,"8023=>80211 err(%d) skblen %d,%p\n",res,ATBM_OS_SKB_LEN(skb),skb);
		atbm_dev_kfree_skb(skb);
		return;
	}
