...
FLASH_FUNC atbm_void atbmwifi_ieee80211_channel_country(struct atbmwifi_common *hw_priv,int country){
...
*/
FLASH_FUNC int  atbmwifi_ieee80211_forward_skb(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
	...
	if((priv->iftype == ATBM_NL80211_IFTYPE_AP) || (priv->iftype == ATBM_NL80211_IFTYPE_P2P_GO)){
        ...
        else if(memcmp(hdr->h_dest,priv->mac_addr,6)){
...
FLASH_FUNC atbm_void atbmwifi_ieee80211_deliver_skb(struct atbmwifi_vif *priv,struct atbm_buff *skb,atbm_uint16 *need_free)
{
	if(!eapol_input(priv,skb)){
		if(priv->assoc_ok==1){
		//if ap mode ,we need forward mul-data or unicast data for other sta 
			if(atbmwifi_ieee80211_forward_skb(priv,skb)){	
...
*/
FLASH_FUNC int atbmwifi_ieee80211_rx_h_amsdu(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
...
FLASH_FUNC int atbmwifi_ieee80211_data_to_8023(struct atbm_buff *skb, const atbm_uint8 *addr,
			   enum atbm_nl80211_iftype iftype)
{
	...
	case atbm_cpu_to_le16(ATBM_IEEE80211_FCTL_TODS):
		if (atbm_unlikely(iftype != ATBM_NL80211_IFTYPE_AP &&
			     iftype != ATBM_NL80211_IFTYPE_P2P_GO))
			return -1;
		break;
	...
	case atbm_cpu_to_le16(ATBM_IEEE80211_FCTL_FROMDS):
		if ((iftype != ATBM_NL80211_IFTYPE_STATION &&
		     iftype != ATBM_NL80211_IFTYPE_P2P_CLIENT ) ||
...
FLASH_FUNC int atbmwifi_ieee80211_data_from_8023(struct atbm_buff *skb, const atbm_uint8 *addr,
...
FLASH_FUNC atbm_void atbmwifi_ieee80211_get_sta_rateinfo(struct atbmwifi_cfg80211_rate *sta,atbm_uint8 *supp_rates,int supp_rates_len)
{
	int i,j;
	int rate =0;
	ATBM_BOOL is_basic;
...
*/
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_rate_ie(atbm_uint8 * pos,ATBM_BOOL no_cck,atbm_uint32 mask)
{
...
FLASH_FUNC  unsigned int atbm_cfg80211_classify8021d(struct atbm_buff *skb)
{
	...
	if (protocol==atbm_htons(ATBM_ETH_P_IP)){
		...
		iphdr = (struct ip_hdr *)(((atbm_uint8 *)ATBM_OS_SKB_DATA(skb))+ATBM_ETH_HLEN);
...
FLASH_FUNC atbm_void ieee802_11_parse_elems(atbm_uint8 *start, int len,
		...
		case ATBM_WLAN_EID_VENDOR_SPECIFIC:
			if (elen >= 4 && pos[0] == 0x00 && pos[1] == 0x50 &&
				...
				if (pos[3] == 1) {
					...
					/* OUI Type 4 - WPS IE , wps IE len may > 255*/
					if(elems->wps_ie == ATBM_NULL){
						elems->wps_ie = pos;
						elems->wps_ie_len = elen;
					}
					else {
						//elems->wps_ie2 = pos;
						//elems->wps_ie_len2 = elen;
					}
				}
			}
...
FLASH_FUNC atbm_uint8 *cfg80211_find_ie(atbm_uint8 eid, const atbm_uint8 *ies, int len)
{
...
#if 0
FLASH_FUNC atbm_uint8 *cfg80211_find_vendor_ie(atbm_uint8 * oui,
				  const atbm_uint8 *ies, int len)
{
	...
	while (pos < end) {
		pos = cfg80211_find_ie(ATBM_WLAN_EID_VENDOR_SPECIFIC, pos,
				       end - pos);
		if (!pos)
			return ATBM_NULL;
 ...
 * Response frames.
 */
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_wmm_param(struct atbmwifi_vif *priv, atbm_uint8 *eid)
{
	atbm_uint8 *pos = eid;
 ...
 */
FLASH_FUNC atbm_uint8 *atbmwifi_ieee80211_add_wme(struct atbmwifi_vif *priv, atbm_uint8 *eid)
{
	atbm_uint8 *pos = eid;
...
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_ht_operation(struct atbmwifi_vif *priv, atbm_uint8 *eid)
{
#if BW_40M_SUPPORT
	...
	*pos++ = ATBM_WLAN_EID_HT_INFORMATION;
	*pos++ = sizeof(*oper);
	...
	oper->control_chan = priv->hw_priv->channel_idex ;
...
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_ht_ie(struct atbmwifi_vif *priv,atbm_uint8 * pos)
{
...
FLASH_FUNC atbm_void atbmwifi_ieee80211_send_deauth_disassoc(struct atbmwifi_vif *priv,
...
FLASH_FUNC atbm_void atbmwifi_ieee80211_connection_loss(struct atbmwifi_vif *priv)
{
	...
	iot_printf(ATBM_KERN_DEBUG "Connection to AP beacon lost.\n");
...
FLASH_FUNC atbm_void atbmwifi_ieee80211_ht_cap_ie_to_sta_ht_cap(struct atbmwifi_ieee80211_supported_band *sband,
...
atbm_void atbmwifi_ieee80211_parse_qos(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
	struct atbmwifi_ieee80211_hdr *hdr = (struct atbmwifi_ieee80211_hdr *)ATBM_OS_SKB_DATA(skb);
	struct atbmwifi_ieee80211_rx_status *status = ATBM_IEEE80211_SKB_RXCB(skb);
	atbm_uint8 tid, seqno_idx, security_idx;
...
FLASH_FUNC int atbmwifi_set_channel_work(atbm_void *work)
{
...
FLASH_FUNC atbm_void atbmwifi_sw_chntimeout(TIMER_1ST_PARAM atbm_void *arg)
{
...
FLASH_FUNC atbm_void atbm_ieee802_11_parse_elems(atbm_uint8 *start, int len,
			       struct atbmwifi_ieee802_11_elems *elems)
{
	atbm_size_t left = len;
	atbm_uint8 *pos = start;
...
FLASH_FUNC int atbm_wifi_reserve_key_ie(atbm_uint8 *buf, int buf_len, atbm_uint8 *ie, int ie_len){
					  ...
					  atbm_uint64 timestamp)
{
	...
	if (new_ch_num!=bss->channel_num){
		...
		if (count <= 1) {
			...
			atbm_StartTimer(&priv->chswitch_timer,
				  atbm_GetOsTimeMs() + (count *priv->bss.beacon_interval));
		}
	}
}
...
ATBM_BOOL atbmwifi_iee80211_check_combination(struct atbmwifi_vif *ignore_priv,atbm_uint8 combination_channel)
{
	...
	for(index = 0;index<ATBM_WIFI_MAX_VIFS;index++){
		...
		wifi_printk(WIFI_ALWAYS,"check_combination vif[%d],join_status(%d),channel(%d),combination_channel(%d)\n",
			priv->if_id,priv->join_status,priv->config.channel_index,combination_channel);
		if((priv->join_status == ATBMWIFI__JOIN_STATUS_STA)||
		   (priv->join_status == ATBMWIFI__JOIN_STATUS_AP)){
		   wifi_printk(WIFI_ALWAYS,"check_combination1++++channel_index(%d),combination_channel(%d)\n",
		   	priv->config.channel_index,combination_channel);
		   if((atbm_uint8)priv->config.channel_index != combination_channel){
		   	wifi_printk(WIFI_ALWAYS,"check_combination1 ATBM_FALSE\n");
		   		return ATBM_FALSE;
		   }
		}
	}
...
atbm_void atbmwifi_stop_iftype(struct atbmwifi_vif *stop_priv,enum atbm_nl80211_iftype stop_type)
{
	...
	wifi_printk(WIFI_ALWAYS,"atbmwifi_stop_iftype:if_type(%d),stop_type(%d)\n",stop_priv->iftype,stop_type);
...
static atbm_void __atbmwifi_start_wifimode(struct atbmwifi_vif *start_priv,enum atbm_nl80211_iftype start_type)
{
	...
	wifi_printk(WIFI_ALWAYS,"atbmwifi_start_wifimode:start_type(%d),old_type(%d),need_change(%d)\n",start_type,start_priv->iftype,priv_need_change);
...
static atbm_void __atbmwifi_stop_wifimode(struct atbmwifi_vif *stop_priv,enum atbm_nl80211_iftype stop_type)
{
	...
	wifi_printk(WIFI_ALWAYS,"atbmwifi_stop_wifimode:other_alive_if(%d),all_alive_if(%d)\n ",other_alive_if,all_alive_if);
	if(all_alive_if != other_alive_if){
		wifi_printk(WIFI_ALWAYS,"all if is down,wpa_timer_free\n ");
	}
}
...
int hexstr2bin(atbm_uint8 *buf, const char *hex, atbm_size_t len)
{
	atbm_size_t i;
	int a;
	const char *ipos = hex;
...
char *dup_binstr(const atbm_void *src, atbm_size_t len)
{
	...
	if(src == ATBM_NULL)
		return ATBM_NULL;
	res = atbm_kmalloc(len + 1,GFP_KERNEL);
	if(res == ATBM_NULL)
