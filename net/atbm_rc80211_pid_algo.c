...
FLASH_FUNC  atbm_uint8  mcs_support(struct atbmwifi_cfg80211_rate  *sta, atbm_uint8 index)
{	
...
FLASH_FUNC  int rate_supported(struct atbmwifi_cfg80211_rate  *sta,int index)
{
	return ((sta == ATBM_NULL) || (sta->support_rates & BIT(index)));
}

FLASH_FUNC int  rate_control_pid_rate_support(struct rc_pid_sta_info *spinfo,struct atbmwifi_cfg80211_rate  *sta,
				 int index)
{
	if(spinfo->b_ht_sta){
...
FLASH_FUNC static atbm_void rate_control_pid_adjust_rate(struct rc_pid_info *pinfo,
...
FLASH_FUNC static atbm_void rate_control_pid_sample(struct rc_pid_info *pinfo,
				    struct atbmwifi_cfg80211_rate  *sta,
...
static NORELOC const atbm_uint16   BitsPerSymbol[RATE_INDEX_MAX] =
...
FLASH_FUNC static atbm_void  rate_control_pid_tx_status(struct atbmwifi_cfg80211_rate  *sta, atbm_void  *priv_sta,struct txrate_status  * status)
{
...
//	int adj=0;
	...
	for(i=0;i<RATE_INDEX_MAX;i++){
		...
		//if(txthroughput[i] !=0){
		//	iot_printf("rate[%d] throughput %x,%d %d\n",i,txthroughput[i],status->txcnt[i],status->txfail[i]);
		//}
		if(spinfo->b_ht_sta){
	...
	/*if(rateidx !=  spinfo->txrate_idx)*/{
		if(spinfo->b_ht_sta){
			...
			//maxrateidx = pinfo->max_rate_index;
				...
				//iot_printf("change ht rate to %d\n",spinfo->txrate_idx);
			}
		}
		else {
				//maxrateidx = pinfo->max_rate_index;
				spinfo->txrate_idx = max_rate;
				//iot_printf("change rate to %d\n",max_rate);
		}
	}
}


FLASH_FUNC ATBM_BOOL rate_control_send_low(struct atbmwifi_ieee80211_tx_info *txrc,struct rc_pid_sta_info *spinfo)
{
	...
	if (txrc->flags & ATBM_IEEE80211_TX_CTL_USE_MINRATE) {		
		...
		txrc->hw_rate_id = hmac_cfg.hw_fix_rate_id;
		txrc->control.rates[0].idx = txrc->hw_rate_id ;
...
FLASH_FUNC static atbm_void rate_control_pid_alloc()
{
...
FLASH_FUNC static atbm_void rate_control_pid_free()
{
...
FLASH_FUNC static atbm_void *rate_control_pid_alloc_sta()
{
	struct rc_pid_sta_info *spinfo;
	int i=0;
...
FLASH_FUNC static atbm_void rate_control_pid_free_sta(atbm_void *priv_sta)
{
	struct rc_pid_sta_info *spinfo = priv_sta;
	spinfo->b_valid = 0;
...
FLASH_RDATA struct rate_control_ops * rate_control_ops_pid_init(atbm_void)
{		
