...
extern int aes_wrap(const atbm_uint8 *kek, int n, const atbm_uint8 *plain, atbm_uint8 *cipher);
extern int aes_unwrap(const atbm_uint8 *kek, int n, const atbm_uint8 *cipher, atbm_uint8 *plain);
...
extern int eloop_register_timeout(unsigned int secs, unsigned int msecs,
			   atbm_void (*handler)(atbm_void *eloop_ctx, atbm_void *timeout_ctx),
			   atbm_void *eloop_data, atbm_void *user_data);
extern int eloop_cancel_timeout(atbm_void (*handler)(atbm_void *eloop_ctx, atbm_void *sock_ctx),
			 atbm_void *eloop_data, atbm_void *user_data);
extern int wpa_timer_init(atbm_void);
//extern atbm_void * os_realloc_array(atbm_void *ptr, atbm_size_t nmemb, atbm_size_t size);
extern int os_get_random(unsigned char *buf, atbm_size_t len);
extern int os_random(atbm_void);
extern atbm_void inc_byte_array(atbm_uint8 *counter, atbm_size_t len);
