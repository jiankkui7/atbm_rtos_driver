...
atbm_minstrel_alloc_sta()
{
	...
	mi = atbm_kmalloc(sizeof(struct atbmwifi_minstrel_sta_info),GFP_KERNEL);
	if (!mi)
		return ATBM_NULL;
	atbm_memset(mi,0,sizeof(struct atbmwifi_minstrel_sta_info));

	max_rates = atbmwifi_band_2ghz.n_bitrates;
	mi->r = atbm_kmalloc(sizeof(struct minstrel_rate) * max_rates,GFP_KERNEL);
	if (!mi->r)
		goto error;

	mi->sample_table = atbm_kmalloc(SAMPLE_COLUMNS * max_rates,GFP_KERNEL);
	if (!mi->sample_table)
		goto error1;
...
FLASH_RDATA atbm_void atbm_rate_control_minstrel_init(atbm_void)
{		
