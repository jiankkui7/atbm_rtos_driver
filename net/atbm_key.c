...
FLASH_FUNC atbm_void atbmwifi_free_key(struct atbmwifi_common *hw_priv, int idx)
{
	//ATBM_BUG_ON(!(hw_priv->key_map & BIT(idx)));
	atbm_memset(&hw_priv->keys[idx], 0, sizeof(hw_priv->keys[idx]));
	hw_priv->key_map &= ~BIT(idx);
}
FLASH_FUNC static atbm_uint8 atbmwifi_crypto_2_type(atbm_uint32 crypto,atbm_uint8 pairwise)    
{
...
int atbm_get_key(struct atbmwifi_vif *priv,int pairwise,int linkid)
{
	...
	int  index = 0;
	key_type = atbmwifi_crypto_2_type(atbm_get_crypto(priv,pairwise),pairwise);
...
FLASH_FUNC struct wsm_add_key *atbmwifi_get_key_buff(struct atbmwifi_vif *priv,int pairwise,int linkid)
{
	struct atbmwifi_key_t *keybuff = ATBM_NULL;
	...
	empty=atbm_get_key(priv,pairwise, linkid);
...
FLASH_FUNC int atbmwifi_set_key(struct atbmwifi_vif *priv,int pairwise,int linkid)
{
	...
	{		
 		struct wsm_add_key *wsm_key = atbmwifi_get_key_buff(priv,pairwise,linkid); //= &hw_priv->keys[idx];
		...
		switch (atbm_get_crypto(priv,pairwise)) {
			case ATBM_WLAN_CIPHER_SUITE_WEP40:
			case ATBM_WLAN_CIPHER_SUITE_WEP104:
...
*/
		...
		//iot_printf("wsm_key->wepPairwiseKey.keyData=%s\n",wsm_key.wepPairwiseKey.keyData);
		//iot_printf("wsm_key->wepGroupKey.keyData=%s\n",wsm_key.wepPairwiseKey.keyData);
		ret = wsm_add_key(hw_priv, wsm_key, priv->if_id);
		if (!ret)
		{
			wifi_printk(WIFI_DBG_MSG,"atbmwifi_set_key ok\n");
//			priv->connect.key_idx=idx;
		}
		else
		{
			wifi_printk(WIFI_DBG_MSG,"atbmwifi_set_key err\n");
//			atbmwifi_free_key(hw_priv, idx);
		}
	}
	return ret;
}
...
{	
	...
	///////////
	entryIndex = atbm_get_key(priv, pairwise,linkid);
...
FLASH_FUNC atbm_void atbmwifi_wep_key_work(struct atbmwifi_vif *priv)
{
