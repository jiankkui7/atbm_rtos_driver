...
atbm_minstrel_ht_alloc_sta(atbm_void)
{
	...
	msp = atbm_kmalloc(sizeof(struct atbm_minstrel_ht_sta_priv),GFP_KERNEL);
	if (!msp)
		return ATBM_NULL;

	msp->ratelist = atbm_kmalloc(sizeof(struct minstrel_rate) * max_rates,GFP_KERNEL);
	if (!msp->ratelist)
		goto error;
	msp->sample_table = atbm_kmalloc(SAMPLE_COLUMNS * max_rates,GFP_KERNEL);
	if (!msp->sample_table)
		goto error1;
...
FLASH_RDATA struct rate_control_ops * atbm_rate_control_minstrel_ht_init(atbm_void)
{			
	...
	struct atbmwifi_ieee80211_tx_rate *rates, int count)
{
	...
	//wifi_printk(WIFI_CONNECT,"[TX policy] Policy (%d): " 
		//"%d:%d, %d:%d, %d:%d, %d:%d, %d:%d\n",
	//	count,
