				...
				const atbm_uint8 *session_mac, const atbm_uint8 *adv_mac)
{
	...
	if (!p2p->p2ps_prov) {
		p2p->p2ps_prov = atbm_kzalloc(sizeof(struct p2ps_provision) + 1, GFP_KERNEL);
		if (!p2p->p2ps_prov)
			return -1;
	} else {
...
out:
	...
	if (!p2p->cfg->p2ps_prov_complete) {
		...
		char *buf = atbm_kmalloc(2 * buf_len + 1, GFP_KERNEL);
				...
				atbm_uint8 *data, size_t len)
{
	...
	if (status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
		...
		if (msg.session_info && msg.session_info_len) {
			...
			char *deferred_sess_resp = atbm_kmalloc(2 * info_len + 1, GFP_KERNEL);
