...
static void p2p_process_presence_req(struct p2p_data *p2p, const atbm_uint8 *da,
				     const atbm_uint8 *sa, const atbm_uint8 *data, size_t len,
				     int rx_freq);
...
void p2p_set_timeout(struct p2p_data *p2p, unsigned int sec, unsigned int usec)
{
	...
	eloop_register_timeout(sec, usec/1000, p2p_state_timeout, p2p, NULL);
}
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	if (os_get_random((atbm_uint8 *) &r, sizeof(r)) < 0)
		r = 0;
					     ...
					     const atbm_uint8 *addr)
{
	...
	dev = atbm_kzalloc(sizeof(*dev), GFP_KERNEL);
	if (dev == NULL)
		return NULL;
...
static int p2p_add_group_clients(struct p2p_data *p2p, const atbm_uint8 *go_dev_addr,
				 ...
				 const atbm_uint8 *gi, size_t gi_len,
...
int p2p_connect(struct p2p_data *p2p, const atbm_uint8 *peer_addr,
		...
		const atbm_uint8 *force_ssid, size_t force_ssid_len,
...
void p2p_build_ssid(struct p2p_data *p2p, atbm_uint8 *ssid, size_t *ssid_len)
{
...
static int p2p_rx_p2p_action(struct p2p_data *p2p, const atbm_uint8 *sa,
			      atbm_uint8 *data, size_t len, int rx_freq)
{
...
static void p2p_rx_action_public(struct p2p_data *p2p, const atbm_uint8 *da,
				 const atbm_uint8 *sa, const atbm_uint8 *bssid, const atbm_uint8 *data,
				 size_t len, int freq)
{
	if (len < 1)
		return;
...
void p2p_rx_action(struct p2p_data *p2p, const atbm_uint8 *da, const atbm_uint8 *sa,
		   ...
		   const atbm_uint8 *data, size_t len, int freq)
{
...
p2p_reply_probe(struct p2p_data *p2p, const atbm_uint8 *addr, const atbm_uint8 *dst,
		const atbm_uint8 *bssid, const atbm_uint8 *ie, size_t ie_len,
...
static int p2p_assoc_req_ie_wlan_ap(struct p2p_data *p2p, const atbm_uint8 *bssid,
				    atbm_uint8 *buf, size_t len, struct wpabuf *p2p_ie)
{
	struct wpabuf *tmp;
	atbm_uint8 *lpos;
	size_t tmplen;
...
int p2p_scan_result_text(const atbm_uint8 *ies, size_t ies_len, char *buf, char *end)
{
			...
			const char *svc_info, const atbm_uint8 *cpt_priority)
{
	...
	adv_data = atbm_kzalloc(adv_data_len, GFP_KERNEL);
	if (!adv_data)
		return -1;
...
int p2p_parse_dev_addr(const atbm_uint8 *ies, size_t ies_len, atbm_uint8 *dev_addr)
{
	struct wpabuf *p2p_ie;
	int ret;
...
struct p2p_data * p2p_init(const struct p2p_config *cfg)
{
	...
	p2p = atbm_kzalloc(sizeof(*p2p) + sizeof(*cfg), GFP_KERNEL);
	if (p2p == NULL)
		return NULL;
	...
	if (cfg->pref_chan) {
		p2p->cfg->pref_chan = atbm_kmalloc(cfg->num_pref_chan *
...
static struct wpabuf * p2p_build_presence_resp(atbm_uint8 status, const atbm_uint8 *noa,
					       size_t noa_len, atbm_uint8 dialog_token)
{
	struct wpabuf *resp;
	atbm_uint8 *len;
...
int p2p_set_ssid_postfix(struct p2p_data *p2p, const atbm_uint8 *postfix, size_t len)
{
