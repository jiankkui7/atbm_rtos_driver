...
void printf_encode(char *txt, size_t maxlen, const atbm_uint8 *data, size_t len)
{
	char *end = txt + maxlen;
	size_t i;
...
const char * wpa_ssid_txt(const atbm_uint8 *ssid, size_t ssid_len)
{
...
void *os_realloc_array(void *ptr, size_t nmemb, size_t size)
{
	...
	return atbm_kzalloc(nmemb * size, GFP_KERNEL);
}
...
{
	void *r = atbm_kmalloc(len, GFP_KERNEL);
...
struct wpabuf * ieee802_11_vendor_ie_concat(const atbm_uint8 *ies, size_t ies_len,
