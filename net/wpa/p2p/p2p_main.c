...
static void wpas_p2p_join_scan_req(struct atbmwifi_vif *priv, int freq,
				   const atbm_uint8 *ssid, size_t ssid_len);
...
int atbm_p2p_connect(struct atbmwifi_vif *priv, const atbm_uint8 *peer_addr,
			 ...
			 const atbm_uint8 *group_ssid, size_t group_ssid_len);
...
void p2p_scan_result(void)
{
	...
	if(priv->p2p_join){
		...
		atbm_os_queueSend(priv->p2p_task_msg, sizeof(struct p2p_task_msg), &msg, 0xffffffff);
		return;
	}
...
//	eloop_cancel_timeout(wpas_send_action_timeout, ctx, NULL);
//	eloop_register_timeout(0, wait_time, wpas_send_action_timeout, priv, NULL);
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	param_res = atbm_kmalloc(sizeof(struct p2p_go_neg_results), GFP_KERNEL);
	if(param_res == NULL){
...
void wpas_sd_request(void *ctx, int freq, const atbm_uint8 *sa, atbm_uint8 dialog_token,
		     atbm_uint16 update_indic, const atbm_uint8 *tlvs, size_t tlvs_len)
{
	//广告服务，不支持
}
void wpas_sd_response(void *ctx, const atbm_uint8 *sa, atbm_uint16 update_indic,
		      const atbm_uint8 *tlvs, size_t tlvs_len)
{
	//广告服务，不支持
}
...
static atbm_uint8 wpas_invitation_process(void *ctx, const atbm_uint8 *sa, const atbm_uint8 *bssid,
				  ...
				  size_t ssid_len, int *go, atbm_uint8 *group_bssid,
...
static void wpas_presence_resp(void *ctx, const atbm_uint8 *src, atbm_uint8 status,
			       const atbm_uint8 *noa, size_t noa_len)
{
...
static int wpas_get_persistent_group(void *ctx, const atbm_uint8 *addr, const atbm_uint8 *ssid,
				     size_t ssid_len, atbm_uint8 *go_dev_addr,
				     atbm_uint8 *ret_ssid, size_t *ret_ssid_len,
				     atbm_uint8 *intended_iface_addr)
{
	return 0;
}
static int wpas_get_go_info(void *ctx, atbm_uint8 *intended_addr,
			    atbm_uint8 *ssid, size_t *ssid_len, int *group_iface,
			    unsigned int *freq)
{
	//获取GO信息
	return 0;
...
static void wpas_p2ps_prov_complete(void *ctx, atbm_uint8 status, const atbm_uint8 *dev,
				    ...
				    const atbm_uint8 *feat_cap, size_t feat_cap_len,
				    unsigned int freq,
				    const atbm_uint8 *group_ssid, size_t group_ssid_len)
{
...
static void wpas_p2p_assoc_req_ie(struct atbmwifi_vif *priv, const atbm_uint8 *bssid, int p2p_group)
{
	...
	priv->p2p_assoc_req_ie = atbm_kmalloc(100, GFP_KERNEL);
...
struct p2p_group *wpas_p2p_group_init(struct atbmwifi_vif *priv, struct p2p_go_neg_results *res)
{
	...
	iot_printf("bbbbbbbbbbbbbbbbbb");
	cfg = atbm_kzalloc(sizeof(*cfg), GFP_KERNEL);
	if (cfg == NULL){
		return NULL;
	}
	iot_printf("ccccccccccccccccccccc");	
...
//		hmac_cfg->dev_password_len = 0; 
		...
		printf("res->wps_method:%d\n", res->wps_method);
		if(res->wps_method == WPS_PBC){
			priv->ssid_length = res->ssid_len;
...
static void p2p_main_thread(void *arg)
{
	...
	while(1){
		...
		ret = atbm_os_queueReceive(priv->p2p_task_msg, &size, &msg, 0xffffffff);
		if(ret || size <= 0){
			continue;
		}
...
static int p2p_main_task_creat(struct atbmwifi_vif *priv)
{
	...
	priv->p2p_task_msg = (atbm_void *)atbm_os_queueCreate(P2P_QUEUE_NUM, sizeof(struct p2p_task_msg));
	if(priv->p2p_task_msg == NULL){
		printf("p2p task msg create err\n");
	}
#if 0
	//create task1
	p2p_thread.fn 		= p2p_main_thread;
	p2p_thread.name 	= "P2P_TASK";
	p2p_thread.ptos		= NULL;
	p2p_thread.stack_size = P2P_TASK_STK_SIZE;
	p2p_thread.arg 		= (void *)priv;
	p2p_thread.prio 	= P2P_TASK_PRIORITY;
	p2p_thread.task		= NULL; 
#endif
...
static int p2p_main_task_release(struct atbmwifi_vif *priv)
{
	...
	if(priv->p2p_task_msg){
		atbm_os_queueDelete(priv->p2p_task_msg);
		priv->p2p_task_msg = NULL;
	}

	if(priv->p2p_task){
//		p2p_thread.task = priv->p2p_task;
		atbm_stopThread(priv->p2p_task);
		atbm_stopThread(p2p_thread);
		priv->p2p_task = NULL;
	}
...
int atbm_p2p_prash_ie(struct atbmwifi_vif *priv, atbm_uint8 *bssid, 
	...
	bss = atbm_kmalloc(sizeof(struct p2p_bss), GFP_KERNEL);
	if(bss == NULL){
		return -1;
	}
	bss->res = atbm_kmalloc(len, GFP_KERNEL);
	if(bss->res == NULL){
		atbm_kfree(bss);
		return -1;
	}
...
int atbm_p2p_probe_req_rx(struct atbmwifi_vif *priv, const atbm_uint8 *addr,
			  ...
			  const atbm_uint8 *ie, size_t ie_len,
...
void atbm_p2p_rx_action(struct atbmwifi_vif *priv, const atbm_uint8 *da,
			...
			atbm_uint8 category, const atbm_uint8 *data, size_t len, atbm_uint16 freq)
{
	struct p2p_data *p2pdata = (struct p2p_data *)priv->p2pdata;
	...
	action = atbm_kmalloc(sizeof(struct p2p_action), GFP_KERNEL);
	if(action == NULL){
		return;
	}
	action->data = atbm_kmalloc(len, GFP_KERNEL);
	if(action->data == NULL){
		atbm_kfree(action);
		return;
	}
...
void atbm_p2p_tx_action_ack(struct atbmwifi_vif *priv, struct atbmwifi_ieee80211_mgmt *mgmt, int status)
{
	...
	action_ack = atbm_kmalloc(sizeof(struct p2p_action_ack), GFP_KERNEL);
	if(action_ack == NULL){
		return;
	}
...
int atbm_p2p_find(struct atbmwifi_vif *priv, unsigned int timeout,
		  ...
		  atbm_uint8 seek_cnt, const char **seek_string, int freq)
{
	struct p2p_data *p2pdata = (struct p2p_data *)priv->p2pdata;
...
int atbm_p2p_go_start(struct atbmwifi_vif *priv)
{
	...
	res = malloc(sizeof(struct p2p_go_neg_results));
	if(res == NULL){
		return -1;
	}
...
//	printf("wps ap ssid:%s,pwd:%s, channel:%d\n", 
//		res->ssid, res->passphrase, hmac_cfg->channel_no);
	atbmwifi_start_iftype(priv, ATBM_NL80211_IFTYPE_AP);
//	wifi_StartAP(res->ssid, res->ssid_len, (atbm_uint8 *)res->passphrase, len, p2pdata->cfg->channel,ATBM_KEY_WPA2, 0);
...
void atbm_p2p_go_stop(struct atbmwifi_vif *priv){
	atbm_disable_listening(priv);
	atbmwps_deinit(priv);
	atbmwifi_stop_iftype(priv, ATBM_NL80211_IFTYPE_AP);
...
{
	priv->p2p_go_intent = go_intent;
	priv->p2p_auto_go_req = 1;
	return atbm_p2p_find(priv, 0, 0, 0, NULL, NULL, 0, 0, NULL, 0);
}

int atbm_p2p_find_only(struct atbmwifi_vif *priv, int timeout)
{
	priv->p2p_auto_go_req = 0;
	return atbm_p2p_find(priv, timeout, 0, 0, NULL, NULL, 0, 0, NULL, 0);
}
