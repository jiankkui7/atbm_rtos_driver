				  ...
				  struct p2p_group_config *config)
{
	...
	group = atbm_kzalloc(sizeof(*group), GFP_KERNEL);
	if (group == NULL)
		return NULL;

	if(!p2p->groups){
		groups = p2p->groups = atbm_kzalloc(P2P_MAX_GROUP_ENTRIES * sizeof(struct p2p_group *), GFP_KERNEL);
	}
			  ...
			  const atbm_uint8 *ie, size_t len)
{
	...
	m = atbm_kzalloc(sizeof(*m), GFP_KERNEL);
	if (m == NULL)
		return -1;
