...
#define WPS_DEVICE_NAME "ALTOBEAM_IOT"
 ...
 */
FLASH_FUNC static atbm_void wpa_config_update_psk(struct atbm_wpa_ssid *ssid)
{
	if (ssid->passphrase) {
 ...
 * @ssid: Pointer to network configuration data
 */
FLASH_FUNC static atbm_void wpa_config_set_network_defaults(struct atbm_wpa_ssid *ssid)
{
...
FLASH_FUNC atbm_void wpa_wps_done(atbm_void *arg,void *data)
{
	struct wpa_supplicant *wpa_s = arg;
	struct atbm_wpa_ssid *ssid = (struct atbm_wpa_ssid *)data;
	struct atbmwifi_vif *priv = (struct atbmwifi_vif *)wpa_get_driver_priv();
	iot_printf("wpa_wps_done\n");
	wpas_deinit_wps(wpa_s);
	if(ssid != NULL){
		atbmwifi_event_uplayer(priv, ATBM_WIFI_WPS_SUCCESS, ssid);
		atbm_kfree(ssid);
		wpa_printf(MSG_ALWAYS, "WPS:wps_cred free %d ssid %p",__LINE__,ssid->ssid);
		if(ssid->ssid != NULL)
...
FLASH_FUNC static int wpa_supplicant_wps_cred(void *ctx,
				   const struct wps_credential *cred)
{
	int ret = 0;
	struct wpa_supplicant *wpa_s = ctx;
	struct atbm_wpa_ssid *ssid;
...
//	struct atbmwifi_vif *priv;
	...
	ssid = atbm_kzalloc(sizeof(*ssid), GFP_KERNEL);
	if(ssid == NULL){
	...
	wpa_printf(MSG_ALWAYS, "WPS: cred, SSID %s len %d",cred->ssid, cred->ssid_len);
	if(cred->key_len == 64){
		wpa_hexdump(MSG_ALWAYS, "WPS: cred, Network Key", cred->key, cred->key_len);
	}else{
		wpa_printf(MSG_ALWAYS, "WPS: cred, Password %s len %d",cred->key, cred->key_len);
	}
	wpa_printf(MSG_ALWAYS, "WPS: cred, Authentication Type 0x%x",cred->auth_type);
	wpa_printf(MSG_ALWAYS, "WPS: cred, Encryption Type 0x%x", cred->encr_type);
	wpa_printf(MSG_ALWAYS, "WPS: cred, Network Key Index %d", cred->key_idx);
	wpa_printf(MSG_ALWAYS, "WPS: cred, MAC Address " MACSTR,MAC2STR(cred->mac_addr));
	...
	ssid->ssid = atbm_kzalloc(cred->ssid_len+1, GFP_KERNEL);
	if (ssid->ssid) {
	...
	case WPS_ENCR_WEP:
		...
		if (cred->key_len == 10 || cred->key_len == 26) {
			if (hexstr2bin(ssid->wep_key[key_idx],
						(char *) cred->key,
				       	cred->key_len / 2) < 0) {
	...
	if (ssid->key_mgmt == ATBM_WPA_KEY_MGMT_PSK) {
		if (cred->key_len == 2 * ATBM_PMK_LEN) {
			if (hexstr2bin(ssid->psk, (const char *) cred->key, ATBM_PMK_LEN)) {
...
//	 AT_WDisConnect(ATBM_NULL);
	...
	eloop_register_timeout(1, 0, wpa_wps_done,wpa_s,ssid);
...
FLASH_FUNC static atbm_void wpa_supplicant_wps_event(void *ctx, enum wps_event event,
 ...
 */
FLASH_FUNC static atbm_void wpa_supplicant_cancel_scan(struct wpa_supplicant *wpa_s)
{
	struct atbmwifi_common *hw_priv = wpa_s->priv->hw_priv;

	if(hw_priv->scan.in_progress){
		wsm_stop_scan(hw_priv,0);
		atbmwifi_scan_comlete(hw_priv);
	}
	atbm_CancelTimer(&wpa_s->priv->scan_expire_timer);
 ...
 */
FLASH_FUNC  atbm_void wps_deinit(struct wps_data *data)
{
 ...
 */
FLASH_FUNC static struct wps_data * wps_init(struct wpa_supplicant *wpa_s)
{
	...
	struct wps_data *data = atbm_kzalloc(sizeof(*data), GFP_KERNEL);
	if (data == NULL){
...
FLASH_FUNC static atbm_void wpas_eap_wsc_deinit(struct wpa_supplicant *wpa_s)
{
 ...
 */
#if 0
FLASH_FUNC static int wpas_eap_wsc_init(struct atbmwifi_vif *priv)
{
	int ret = 0;
	...
	wsc_data = atbm_kzalloc(sizeof(*wsc_data), GFP_KERNEL);
	if (wsc_data == NULL){
 ...
 * This function is called at program initialization of WPS STA. 
 */
FLASH_FUNC static atbm_void wpas_wps_set_uuid(struct wpa_supplicant *wpa_s,
			   ...
			   const char *pin, int p2p_group, atbm_uint16 dev_pw_id)
{
	...
	priv = (struct atbmwifi_vif *)wpa_get_driver_priv();
...
extern int msh_exec(char *cmd, rt_size_t length);
	...
	wpa_s->pin = atbm_kzalloc(PIN_CODE_LENGTH, GFP_KERNEL);
	wpa_s->scan_runs = 0;
...
FLASH_FUNC int wpas_wps_p2p_init(struct wpa_supplicant *wpa_s)
{
	int ret = 0;
...
FLASH_FUNC int wpas_wps_p2p_start_pbc(struct wpa_supplicant *wpa_s, const atbm_uint8 *bssid, int p2p_group)
{
	struct atbmwifi_vif *priv = wpa_s->priv;
