...
FLASH_FUNC int random_get_bytes(atbm_uint8 *buf, atbm_size_t len)
{
 ...
 */
FLASH_FUNC struct wpabuf * wps_build_probe_req_ie(atbm_uint16 pw_id, struct wps_device_data *dev,
...
extern FLASH_FUNC atbm_void sys_dump_mem(const atbm_void *pMem, int size);
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_preq_wps_ie(struct atbmwifi_vif *priv,atbm_uint8 * pos)
{
...
FLASH_FUNC atbm_uint8 * atbmwifi_ieee80211_add_assocreq_wps_ie(struct atbmwifi_vif *priv,atbm_uint8 * pos)
{
	struct wpa_supplicant *wpa_s;
	struct wpabuf *wps_ie;
...
FLASH_FUNC static int wps_set_vendor_ext_wfa_subelem(struct wps_parse_attr *attr,
...
FLASH_FUNC static int wps_parse_vendor_ext_wfa(struct wps_parse_attr *attr, const atbm_uint8 *pos,
					atbm_uint16 len)
{
...
FLASH_FUNC  int wps_parse_vendor_ext(struct wps_parse_attr *attr, const atbm_uint8 *pos,
				atbm_uint16 len)
{
...
FLASH_FUNC int wps_parse_msg_attr(const atbm_uint8 *msg, atbm_uint8 length, struct wps_parse_attr *attr)
{
...
FLASH_FUNC static const char * eap_wsc_state_txt(int state)
{
	switch (state) {
	case WAIT_START:
		return "WAIT_START";
...
FLASH_FUNC atbm_void eap_wsc_state(struct eap_wsc_data *data, int state)
{
...
FLASH_FUNC static int wpa_supplicant_ap_wps_pin(struct hostapd_data *hapd, const atbm_uint8 *bssid,
 ...
 */
FLASH_FUNC struct wpabuf * wps_get_msg(struct wps_data *wps, enum wsc_op_code *op_code)
{
	if (wps->registrar)
 ...
 */
FLASH_FUNC enum wps_process_res wps_process_msg(struct wps_data *wps,
...
FLASH_FUNC struct wpabuf * eap_wsc_build_frag_ack(atbm_uint8 id, atbm_uint8 code)
{
	struct wpabuf *msg;
...
FLASH_FUNC static struct wpabuf * eap_wsc_process_fragment(struct eap_wsc_data *data,
...
FLASH_FUNC static int eap_wsc_process_cont(struct eap_wsc_data *data,
...
FLASH_FUNC static struct wpabuf * eap_wsc_build_msg(struct eap_wsc_data *data,atbm_uint8 id)
{
	struct wpabuf *resp;
	atbm_uint8 flags;
...
FLASH_FUNC struct wpabuf * wpas_eap_wsc_process(struct wpa_supplicant *wpa_s, const struct wpabuf *reqData)
{
 ...
 */
FLASH_FUNC int atbmwps_start_pbc(struct atbmwifi_vif *priv, atbm_uint8 *p2p_info)
{
	int ret = 0;
	int p2p_group=0;
 ...
 */
FLASH_FUNC int atbmwps_start_pin(struct atbmwifi_vif *priv, const char *pin, atbm_uint8* buf, atbm_uint32 info)
{
	int ret = 0;
	int p2p_group=0;
	...
	switch(priv->iftype){
		...
		case ATBM_NL80211_IFTYPE_AP:
			ret = wpa_supplicant_ap_wps_pin(priv->appdata, priv->bssid, pin, (char *)buf, info/*buf length*/);
			if(ret != 0)
				break;		
				
			hmac_cfg.dev_password_len = 8;//strlen(pin);	
			memcpy(hmac_cfg.dev_password, pin, 8);	
			priv->pin = 1;
			priv->pbc = 0;
...
/**
 * atbmwps_timeout - the wps pbc/pin requests timeout
 * Returns: 0 on success, -1 on failure
 *
 * This function is called at the wps pbc/pin requests is timeout.
 */
FLASH_FUNC int atbmwps_timeout(void *eloop_ctx, atbm_void *timeout_ctx)
{
	int ret = 0;
	struct atbmwifi_vif *priv;
	
	priv = wpa_get_driver_priv();
	if(priv == NULL){
		wpa_printf(MSG_ERROR, "wps: timeout, priv is not finish init.");
		return -1;
	}
	
	if(priv->iftype == ATBM_NL80211_IFTYPE_AP){
		hostapd_wps_timeout(eloop_ctx, timeout_ctx);
	}else if(priv->iftype == ATBM_NL80211_IFTYPE_STATION){
		wpas_wps_timeout(eloop_ctx, timeout_ctx);
	}else{
		wpa_printf(MSG_ERROR, "wps: timeout, iftype is not support.");
		ret = -1;
	}
 ...
 */
FLASH_FUNC int atbmwps_cancel(struct atbmwifi_vif *priv)
{
	int ret = 0;
 ...
 */
FLASH_FUNC int atbmwps_deinit(struct atbmwifi_vif *priv)
{
	int ret = 0;
 ...
 */
FLASH_FUNC int atbmwps_init(struct atbmwifi_vif *priv)
{
	int ret = 0;
