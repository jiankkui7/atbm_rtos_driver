...
#include "wps_main.h"//"eap_wsc_common.h"//
#include "wps.h"
#include "wps_i.h"
//#include "atbmwifi.h"
#include "atbm_hal.h"
...
FLASH_FUNC atbm_void hostapd_eap_wsc_init(struct atbmwifi_vif *priv,atbm_uint8 *da)
{
	...
	data = atbm_kzalloc(sizeof(struct wps_data), GFP_KERNEL);
	if (data == NULL)
		return;
	...
	//wpa_printf(MSG_DEBUG, "EAP-WSC: eap_wsc_init len=%d\n", hmac_cfg->dev_password_len);
	if (hmac_cfg.dev_password_len > 0) {
		data->dev_pw_id = DEV_PW_DEFAULT;
		data->dev_password = (atbm_uint8 *)strdup((char *)hmac_cfg.dev_password);
		data->dev_password_len = hmac_cfg.dev_password_len;
		wpa_printf(MSG_DEBUG, "WPS: AP PIN dev_password %s %d",
				data->dev_password, data->dev_password_len);
	}
	//wpa_printf(MSG_DEBUG, "EAP-WSC: eap_wsc_init pbc=%d\n", hmac_cfg->pbc);
...
FLASH_FUNC atbm_void eap_wsc_dinit(struct atbmwifi_vif *priv)
{
	struct hostapd_data *hapd;
	hapd = (struct hostapd_data *)priv->appdata;
...
FLASH_FUNC struct wpabuf * eap_wsc_build_start(atbm_uint8 id)
{
	struct wpabuf *req;

	req = eap_msg_alloc(ATBM_EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC, 2,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
...
FLASH_FUNC static struct wpabuf * eap_wsc_build_msg_fail(atbm_uint8 id)
{
	struct wpabuf *req;
...
FLASH_FUNC atbm_void wps_send_eapol_timeout(void *data1, void *data2)
{
	struct hostapd_sta_info *sta = data2;
	if(g_hostapd && sta){
		wifi_printk(WIFI_ALWAYS, "wps_send_eapol_timeout tx retry!!\n");
		hostapd_send_eapol(wpa_get_driver_priv(), sta->addr, ATBM_ETH_P_EAPOL, (atbm_uint8*)g_hostapd->wps_tx_hdr, g_hostapd->wps_tx_hdr_len);
	}
}

FLASH_FUNC atbm_void hostapd_eap_wsc_process(struct hostapd_data *hapd,struct hostapd_sta_info *sta,struct wpabuf *respData)
{
...
//	atbm_uint16 message_length = 0;
	...
	case WPS_DONE:
		...
		eloop_cancel_timeout(wps_send_eapol_timeout, (atbm_void *)hapd, (atbm_void *)sta);
		hapd->priv->pbc = 0;
		hapd->priv->pin = 0;
		hapd->wpsdata->id++;
