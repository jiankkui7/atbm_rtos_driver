...
FLASH_FUNC atbm_void wps_kdf(const atbm_uint8 *key, const atbm_uint8 *label_prefix, atbm_size_t label_prefix_len,
...
FLASH_FUNC atbm_int32 wps_derive_keys(struct wps_data *wps)
{
	...
	dhkey = atbm_kzalloc(SHA256_MAC_LEN, GFP_KERNEL);
	if (dhkey == NULL)
	{
	...
	kdk = atbm_kzalloc(SHA256_MAC_LEN, GFP_KERNEL);
	if (kdk == NULL)
	{
	...
	addr[2] = wps->nonce_r;
	len[2] = WPS_NONCE_LEN;
	hmac_sha256_vector(dhkey, SHA256_MAC_LEN, 3, addr, len, kdk);
	//wpa_hexdump_key(MSG_DEBUG, "WPS: KDK", kdk, SHA256_MAC_LEN);

	keys = atbm_kzalloc(WPS_AUTHKEY_LEN + WPS_KEYWRAPKEY_LEN + WPS_EMSK_LEN, GFP_KERNEL);
	if (keys == NULL)
	{
...
FLASH_FUNC atbm_void wps_derive_psk(struct wps_data *wps, const atbm_uint8 *dev_passwd,
...
FLASH_FUNC struct wpabuf * wps_decrypt_encr_settings(struct wps_data *wps, const atbm_uint8 *encr,
					  atbm_size_t encr_len)
{
	...
	if (aes_128_cbc_decrypt(wps->keywrapkey, encr, wpabuf_mhead(decrypted),
		...
		wpa_printf(MSG_ERROR, "WPS: aes_128_cbc_decrypt err");
 ...
 */
FLASH_FUNC atbm_uint32 wps_pin_checksum(atbm_uint32 pin)
{
	atbm_uint32 accum = 0;
	while (pin) {
		accum += 3 * (pin % 10);
 ...
 */
FLASH_FUNC atbm_uint32 wps_pin_valid(atbm_uint32 pin)
{
 ...
 */
FLASH_FUNC atbm_uint32 wps_generate_pin(atbm_void)
{
	atbm_uint32 val;
...
FLASH_FUNC atbm_int32 wps_pin_str_valid(const char *pin)
{
	const char *p;
	size_t len;
...
FLASH_FUNC atbm_void wps_fail_event(struct wps_context *wps, enum wps_msg_type msg,
...
FLASH_FUNC atbm_void wps_success_event(struct wps_context *wps)
{
	//union wps_event_data data;
...
FLASH_FUNC atbm_void wps_pwd_auth_fail_event(struct wps_context *wps, atbm_int32 enrollee, atbm_int32 part)
{
	union wps_event_data data;
...
FLASH_FUNC atbm_void wps_pbc_overlap_event(struct wps_context *wps)
{
	if (wps->event_cb == NULL)
		return;
...
FLASH_FUNC atbm_void wps_pbc_timeout_event(struct wps_context *wps)
{
	if (wps->event_cb == NULL)
		return;
...
FLASH_FUNC atbm_void wps_pbc_active_event(struct wps_context *wps)
{
	if (wps->event_cb == NULL)
		return;
...
FLASH_FUNC atbm_void wps_pbc_disable_event(struct wps_context *wps)
{
	if (wps->event_cb == NULL)
		return;
...
FLASH_FUNC atbm_int32 wps_dev_type_str2bin(const char *str, atbm_uint8 dev_type[WPS_DEV_TYPE_LEN])
{
	const char *pos;
	...
	if (pos == NULL)
		return -1;
	pos++;
	if (hexstr2bin(&dev_type[2], pos, 4))
		return -1;
	pos = strchr(pos, '-');
	if (pos == NULL)
		return -1;
...
FLASH_FUNC char * wps_dev_type_bin2str(const atbm_uint8 dev_type[WPS_DEV_TYPE_LEN], char *buf,
			    atbm_size_t buf_len)
{
//	atbm_int32 ret;
...
FLASH_FUNC atbm_void uuid_gen_mac_addr(const atbm_uint8 *mac_addr, atbm_uint8 *uuid)
{
	...
	addr[1] = mac_addr;
	len[1] = 6;
	atbm_sha1_vector(2, addr, len, hash);
...
FLASH_FUNC atbm_uint16 wps_config_methods_str2bin(const char *str)
{
 ...
 */
FLASH_FUNC struct wpabuf * wps_build_assoc_resp_ie(atbm_void)
{
	struct wpabuf *ie;
	atbm_uint8 *len;
...
FLASH_FUNC struct wpabuf * wps_build_wsc_ack(struct wps_data *wps)
{
	struct wpabuf *msg;
...
FLASH_FUNC struct wpabuf * wps_build_wsc_nack(struct wps_data *wps)
{
	struct wpabuf *msg;
