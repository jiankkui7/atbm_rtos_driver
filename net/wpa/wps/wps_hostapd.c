...
#if 0
FLASH_FUNC static atbm_void hostapd_wps_event_cb(void *ctx, enum wps_event event,
...
FLASH_FUNC static int hostapd_wps_rf_band_cb(void *ctx)
{
	return WPS_RF_24GHZ; /* FIX: dualband AP */
}

FLASH_FUNC atbm_void hostapd_wps_timeout(void *eloop_ctx, atbm_void *timeout_ctx)
{
...
FLASH_FUNC static atbm_void hostapd_wps_clear_ies(struct hostapd_data *hapd, int deinit_only)
{
...
FLASH_FUNC static atbm_void hostapd_free_wps(struct wps_context *wps)
{
	int i;
 ...
 */
FLASH_FUNC int hostapd_init_wps(struct atbmwifi_vif *priv)
{
	...
	wps = atbm_kzalloc(sizeof(*wps), GFP_KERNEL);
	if (wps == NULL)
	{
	...
	if (hexstr2bin(os_version, "01020300", 4)) {
...
FLASH_FUNC int hostapd_init_wps_complete(struct hostapd_data *hapd)
{
 ...
 */
FLASH_FUNC atbm_void hostapd_deinit_wps(struct hostapd_data *hapd)
{
...
FLASH_FUNC int wps_add_pin(struct hostapd_data *hapd, atbm_void *ctx)
{
	int ret = 0;
...
FLASH_FUNC int hostapd_wps_button_pushed(struct hostapd_data *hapd, atbm_void *ctx)
{
...
FLASH_FUNC int hostapd_cancel_wps(struct hostapd_data *hapd)
{
	if (hapd->wps == NULL)
		return -1;
...
FLASH_FUNC static atbm_void hostapd_wps_ap_pin_timeout(void *eloop_data, atbm_void *user_ctx)
{
...
#if 0
FLASH_FUNC static atbm_void hostapd_wps_ap_pin_enable(struct hostapd_data *hapd, int timeout)
{
}


FLASH_FUNC static int wps_ap_pin_disable(struct hostapd_data *hapd, atbm_void *ctx)
{
	return 0;
}
#endif

FLASH_FUNC atbm_void hostapd_wps_ap_pin_disable(struct hostapd_data *hapd)
{
...
#if 0
FLASH_FUNC int hostapd_wps_config_ap(struct hostapd_data *hapd, char *ssid,
			  const char *auth, const char *encr, const char *key)
{
	...
	if ((len & 1) || len > 2 * sizeof(cred.ssid) ||
	    hexstr2bin((atbm_uint8 *)ssid, (char *)cred.ssid, len / 2))
		return -1;
	cred.ssid_len = len / 2;
	...
	if (key) {
		...
		if ((len & 1) || len > 2 * sizeof(cred.key) ||
		    hexstr2bin(key, (char *)cred.key, len / 2))
			return -1;
		cred.key_len = len / 2;
	}
