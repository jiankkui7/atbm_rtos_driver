...
FLASH_FUNC static atbm_void wpabuf_overflow(const struct wpabuf *buf, atbm_size_t len)
{
...
FLASH_FUNC int wpabuf_resize(struct wpabuf **_buf, atbm_size_t add_len)
{
 ...
 */
FLASH_FUNC struct wpabuf * wpabuf_alloc_1(size_t len,atbm_uint32 call)
{
	struct wpabuf *buf = atbm_kzalloc(sizeof(struct wpabuf) + len, GFP_KERNEL);
		
	if(len > 10*1024){
		iot_printf("wpabuf_alloc too big len %d func %x\n",len,call);
	}
	if (buf == NULL){
		iot_printf("wpabuf_alloc ERR len %d func %x\n",len,call);
		return NULL;
	}
...
FLASH_FUNC struct wpabuf * wpabuf_alloc_ext_data(atbm_uint8 *data, atbm_size_t len)
{
	struct wpabuf *buf = atbm_kzalloc(sizeof(struct wpabuf), GFP_KERNEL);
	if (buf == NULL)
		return NULL;
...
FLASH_FUNC struct wpabuf * wpabuf_alloc_copy(const atbm_void *data, atbm_size_t len)
{
...
FLASH_FUNC struct wpabuf * wpabuf_dup(const struct wpabuf *src)
{
 ...
 * @buf: wpabuf buffer
 */
FLASH_FUNC atbm_void wpabuf_free(struct wpabuf *buf)
{
	if (buf == NULL)
		return;
	if (buf->flags & WPABUF_FLAG_EXT_DATA)
...
FLASH_FUNC atbm_void wpabuf_clear_free(struct wpabuf *buf)
{
	if (buf) {
...
NO_FLASH_FUNC atbm_void wpabuf_put_u8(struct wpabuf *buf, atbm_uint8 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 1);
	*pos = data;
}

NO_FLASH_FUNC atbm_void wpabuf_put_le16(struct wpabuf *buf, atbm_uint16 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 2);
	ATBM_WPA_PUT_LE16(pos, data);
}

NO_FLASH_FUNC atbm_void wpabuf_put_le32(struct wpabuf *buf, atbm_uint32 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 4);
	ATBM_WPA_PUT_LE32(pos, data);
}

NO_FLASH_FUNC atbm_void wpabuf_put_be16(struct wpabuf *buf, atbm_uint16 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 2);
	ATBM_WPA_PUT_BE16(pos, data);
}

NO_FLASH_FUNC atbm_void wpabuf_put_be24(struct wpabuf *buf, atbm_uint32 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 3);
	ATBM_WPA_PUT_BE24(pos, data);
}

NO_FLASH_FUNC atbm_void wpabuf_put_be32(struct wpabuf *buf, atbm_uint32 data)
{
	atbm_uint8 *pos = wpabuf_put(buf, 4);
	ATBM_WPA_PUT_BE32(pos, data);
}

NO_FLASH_FUNC atbm_void wpabuf_put_data(struct wpabuf *buf, const atbm_void *data,
				   atbm_size_t len)
{
	if (data)
...
NO_FLASH_FUNC atbm_void wpabuf_put_buf(struct wpabuf *dst,
				  const struct wpabuf *src)
{
	wpabuf_put_data(dst, wpabuf_head(src), wpabuf_len(src));
}

NO_FLASH_FUNC atbm_void * wpabuf_put(struct wpabuf *buf, atbm_size_t len)
{
 ...
 */
FLASH_FUNC struct wpabuf * wpabuf_concat(struct wpabuf *a, struct wpabuf *b)
{
 ...
 */
FLASH_FUNC struct wpabuf * wpabuf_zeropad(struct wpabuf *buf, atbm_size_t len)
{
	struct wpabuf *ret;
	size_t blen;
