...
//atbm_void * os_realloc_array(atbm_void *ptr, atbm_size_t nmemb, atbm_size_t size)
//{
//	if (size && nmemb > (~(atbm_size_t) 0) / size)
//		return ATBM_NULL;
//	return atbm_realloc(ptr, nmemb * size);
//}
struct atbmwifi_vif * wpa_get_driver_priv(){
	return g_vmac;
}
...
{
	struct atbm_cfg80211_connect_params *connect = &priv->connect;
//	wifi_printk(WIFI_DBG_MSG, "WPA:install gtk\n\r");
	switch (pairwise_cipher) {
				      ...
				      atbm_uint32 pairwise_cipher,atbm_uint16 key_index)
{
	...
	atbm_memset(key_zero,0,connect->key_len );
	if(atbm_memcmp(connect->key,key_zero,connect->key_len)==0){
	...
	atbmwifi_set_key(priv,1,key_index>>8);
...
#
//		wifi_printk(WIFI_DBG_INIT,"ie len short\n");
		return -1;
	}
...
//		wifi_printk(WIFI_DBG_INIT,"malformed ie or unknown version\n");
		return -2;
	}
...
#endif
#ifdef CONFIG_IEEE80211W
	if (conf->atbmwifi_ieee80211w != ATBM_NO_MGMT_FRAME_PROTECTION) {
		capab |= ATBM_WPA_CAPABILITY_MFPC;
		if (conf->atbmwifi_ieee80211w == ATBM_MGMT_FRAME_PROTECTION_REQUIRED)
...
atbm_void wpa_comm_init_extra_ie(struct atbmwifi_vif *priv)
{
	...
	wpa_ie = atbm_kmalloc(500,GFP_KERNEL);
	pos = wpa_ie;
...
//  TO DO ADD WPS IE
	...
	if((wpa_ie != ATBM_NULL)&&( wpa_ie_len != 0))
	{
		...
		if(priv->bss.information_elements == ATBM_NULL){
			priv->bss.information_elements = atbm_kmalloc(priv->extra_ie_len,GFP_KERNEL);
			if(priv->bss.information_elements == ATBM_NULL)
			{	
...
atbm_void inc_byte_array(atbm_uint8 *counter, atbm_size_t len)
{
	int pos = len - 1;
	while (pos >= 0) {
		counter[pos]++;
...
int hmac_md5_vector(const atbm_uint8 *key, atbm_size_t key_len, atbm_size_t num_elem,
		    const atbm_uint8 *addr[], const atbm_size_t *len, atbm_uint8 *mac)
{
	...
	if (num_elem > 5) {
        ...
        if (key_len > 64) {
		if (atbm_md5_vector(1, &key, &key_len, tk))
			return -1;
		key = tk;
		key_len = 16;
        }
	...
	if (atbm_md5_vector(1 + num_elem, _addr, _len, mac))
		return -1;
	...
	_addr[1] = mac;
	_len[1] = MD5_MAC_LEN;
	return atbm_md5_vector(2, _addr, _len, mac);
}

int hmac_md5(const atbm_uint8 *key, atbm_size_t key_len, const atbm_uint8 *data, atbm_size_t data_len,
	      atbm_uint8 *mac)
{
			     ...
			     struct wpa_eapol_ie_parse *ie)
{
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
	    ATBM_RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_NONCE) {
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
	    ATBM_RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_LIFETIME) {
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
	    ATBM_RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_ERROR) {
...
#ifdef CONFIG_IEEE80211W
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
	    ATBM_RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_IGTK) {
...
#ifdef CONFIG_IEEE80211W
	if (use_sha256)
		atbm_sha256_prf(pmk, pmk_len, label, data, sizeof(data),
			   ptk, ptk_len);
	else
#endif /* CONFIG_IEEE80211W */
...
int rc4_skip(const atbm_uint8 *key, atbm_size_t keylen, atbm_size_t skip,
...
int aes_unwrap(const atbm_uint8 *kek, int n, const atbm_uint8 *cipher, atbm_uint8 *plain)
{
 ...
 */
int aes_wrap(const atbm_uint8 *kek, int n, const atbm_uint8 *plain, atbm_uint8 *cipher)
{
	atbm_uint8 *a, *r, b[16];
	int i, j;
	atbm_void *ctx;
...
//	wifi_printk(WIFI_CONNECT,"eapol_input len(%d)\n",ATBM_OS_SKB_LEN(skb));
	...
	atbmwifi_wpa_event_queue((atbm_void*)priv,(atbm_void*)skb,ATBM_NULL,WPA_EVENT__EAP_RX,0);
	return 1;
